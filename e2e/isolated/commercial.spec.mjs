...
test('one commercial chain: purchase, receipt, stock, load, sale, collection and reports', async ({ browser }, info) => {
  ...
  try {
    ...
    await step('COM-01 PWA logins and purchase creation do not create stock',async()=>{
      ...
      await expect(purchase.locator('#orderTotalPreview')).toHaveText('USD 250.00');
      const originalViewport=a.page.viewportSize();
      await a.page.setViewportSize({...originalViewport,height:500});
      await purchase.locator('#orderTitle').scrollIntoViewIfNeeded();
      await screenshot(a,'01-purchase-form');
      expect(await purchase.locator('.purchase-order-dialog').evaluate(el=>el.scrollWidth<=el.clientWidth+1)).toBe(true);
      await purchase.locator('#saveOrder').scrollIntoViewIfNeeded();
      const saveBox=await purchase.locator('#saveOrder').boundingBox();
      expect(saveBox.y).toBeGreaterThanOrEqual(0);
      expect(saveBox.y+saveBox.height).toBeLessThanOrEqual(501);
      await screenshot(a,'01-purchase-save-visible');
      await mutation(a,'purchases',()=>purchase.locator('#saveOrder').click());
      await expect(purchase.locator('#orderModal')).toBeHidden();
      await a.page.setViewportSize(originalViewport);
