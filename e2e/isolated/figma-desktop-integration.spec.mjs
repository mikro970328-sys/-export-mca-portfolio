...
for(const width of [1440,390])test(`Integrated navigation preserves native and embedded viewport at ${width}`,async({page},info)=>{
 ...
 if(width===390){await page.locator('#mobileMenuBtn').click();await expect(page.locator('body')).toHaveClass(/mobile-nav-open/);}await page.locator('#navigationSearch').fill('compras');await expect(page.locator('[data-section="purchasesSection"]')).toBeVisible();await page.locator('[data-section="purchasesSection"]').click();await expect(frame).toBeVisible();if(width===390)await expect(page.locator('body')).not.toHaveClass(/mobile-nav-open/);await shot(page,info,'navegacion-'+width);
});
