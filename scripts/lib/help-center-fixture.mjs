...
export function helpCenterFixture({master=true}={}) {
  ...
  const shell=new JSDOM(readFileSync('admin/index.html','utf8'));
  doc.querySelector('.main-shell main').append(doc.importNode(shell.window.document.getElementById('helpSection'),true));
  const style=doc.createElement('style');
  style.textContent=readFileSync('admin/help-center.css','utf8');doc.head.append(style);
  for(const file of ['admin/help-content.js','admin/help-center.js']){
    const script=doc.createElement('script');script.textContent=readFileSync(file,'utf8').replaceAll('</script','<\\/script');doc.body.append(script);
  }
