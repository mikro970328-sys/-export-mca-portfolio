...
async function hasPermission(admin,permissionKey){if(admin.role==='master_admin')return true;const permissionRows=await rows('admin_effective_permissions',`?select=permission_key&admin_user_id=eq.${encodeURIComponent(admin.admin_id)}&permission_key=eq.${encodeURIComponent(permissionKey)}&limit=1`);return Boolean(permissionRows.length);}

async function workspace(salesOrderId,{documentsReadable=false}={}){
...
export default async function handler(req,res){
  ...
  try{const salesOrderId=requiredUuid(req.query?.sales_order_id||req.query?.id),documentsReadable=await hasPermission(admin,'documents.read'),data=await workspace(salesOrderId,{documentsReadable});if(!data)return fail(res,404,'Sales Order no encontrada');return ok(res,{workspace:data});}
