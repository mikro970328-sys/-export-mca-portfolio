...
async function writeHistory(shipment,event,admin,details,correctionType) {
  try {
    const title = correctionType === 'rollback'
      ...
      : correctionType === 'same'
        ? `Tracking ERP actualizado · ${event.status}`
        : `Tracking ERP · ${event.status}`;
    ...
    console.error('ERP_TRACKING_HISTORY_FAILED',error.message);
  }
}
  ...
  try {
    await supabase('audit_log', {
      ...
      body:[{
        ...
        action:correctionType === 'rollback' ? 'tracking_event_corrected' : 'tracking_event_confirmed',
    ...
    console.error('ERP_TRACKING_AUDIT_FAILED',error.message);
  }
}
...
async function logNotification(shipment,event,data={}) {
  ...
  try {
    ...
    console.error('ERP_TRACKING_NOTIFICATION_LOG_FAILED',error.message);
  }
}

function variablesFor(event,shipment) {
  if (event.eventType === 'DEPA') return { '1':shipment.container_number };
...
function correctionPatch(shipment,eventKey,event,location,admin,now) {
  ...
  if (event.order < EVENTS.released.order) {
    ...
    patch.release_method = 'erp_tracking';
...
async function patchReleaseNotification(shipmentId,status,error=null) {
  await supabase('shipments', {
    method:'PATCH',
    query:`?id=eq.${encodeURIComponent(shipmentId)}`,
    body:{ release_notification_status:status,release_notification_error:error,updated_at:new Date().toISOString() }
  });
}
...
export default async function handler(req,res) {
  ...
  try {
    ...
    if (!shipment.client_id || !shipment.clients?.active || !shipment.clients?.phone) {
      ...
      if (event.eventType === 'RELEASE') await patchReleaseNotification(shipment.id,'unavailable_recipient',recipientError);
    ...
    if (!templateSid) {
      const configError = `Falta ${event.templateEnv} en Vercel`;
      if (event.eventType === 'RELEASE') await patchReleaseNotification(shipment.id,'pending',configError);
      await logNotification(shipment,event,{ status:'pending',templateSid:null,location,error:configError,correctionType });
      await writeHistory(shipment,event,admin,`${correctionDetail} · Confirmado por ${admin.username || 'administrador'} · WhatsApp pendiente: ${configError}`,correctionType);
    ...
    if (!claim.claimed) {
      if (event.eventType === 'RELEASE') await patchReleaseNotification(shipment.id,'already_notified');
    ...
    try {
      ...
      if (event.eventType === 'RELEASE') await patchReleaseNotification(shipment.id,'sent');
      ...
      if (event.eventType === 'RELEASE') await patchReleaseNotification(shipment.id,'failed',error.message);
    ...
    console.error('ERP_TRACKING_EVENT_ERROR',error);
    return fail(res,400,'No se pudo confirmar el evento de tracking',error.message);
  }
}
