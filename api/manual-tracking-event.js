import { authorizeAdmin, fail, ok, readJson, sendWhatsApp, supabase, upstreamFailureStatus } from './_lib.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    let sent;
    try {
      sent=await sendWhatsApp({to:shipment.clients.phone,contentSid:templateSid,variables:variablesFor(event,shipment)});
    } catch(error) {
      ...
      return ok(res,{updated:true,event:eventKey,status:event.status,previous_status:previousStatus,correction_type:correctionType,notification_status:'failed',notification_error:'No se pudo enviar la notificación de WhatsApp',notified:false});
    }
    // Provider acceptance is irreversible: later persistence errors must keep
    // the delivery claim so a retry cannot send the same milestone again.
  ...
  }catch(error){console.error('ERP_TRACKING_EVENT_ERROR',error);const raw=String(error.message||'');if(raw.includes('SHIPMENT_ALREADY_DELIVERED'))return fail(res,409,'Este contenedor ya fue marcado como entregado.');if(raw==='JSON_INVALID')return fail(res,400,'Solicitud inválida');return fail(res,upstreamFailureStatus(error,500),'No se pudo confirmar el evento de tracking');}
}
