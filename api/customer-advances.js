...
async function loadSalesOrderFinance(salesOrderId){
  const [progressRows,advances,invoices]=await Promise.all([
  ...
  for(const row of applications||[]){if(!appsByAdvance.has(row.customer_advance_id))appsByAdvance.set(row.customer_advance_id,[]);appsByAdvance.get(row.customer_advance_id).push({...row,invoice:invoiceById.get(row.invoice_id)||null});}
  const refundsByAdvance=new Map();
  for(const row of refunds||[]){if(!refundsByAdvance.has(row.customer_advance_id))refundsByAdvance.set(row.customer_advance_id,[]);refundsByAdvance.get(row.customer_advance_id).push(row);}
  return {
    progress:progressRows?.[0]||null,
    advances:(advances||[]).map(row=>({...row,applications:appsByAdvance.get(row.customer_advance_id)||[],refunds:refundsByAdvance.get(row.customer_advance_id)||[]})),
...
export default async function handler(req,res){
  ...
  try{
    if(req.method==='GET'){
      ...
      return ok(res,await loadSalesOrderFinance(salesOrderId));
    }
    ...
    if(action==='register'){
      ...
      return ok(res,{advance:row,finance:await loadSalesOrderFinance(salesOrderId)});
    }
    if(action==='apply'){
      ...
      return ok(res,{application:row,finance:advance?.sales_order_id?await loadSalesOrderFinance(advance.sales_order_id):null});
    }
    if(action==='refund'){
      ...
      return ok(res,{refund:row,finance:advance?.sales_order_id?await loadSalesOrderFinance(advance.sales_order_id):null});
    }
    if(action==='reverse'){
      const advanceId=uuid(body.customer_advance_id,'CUSTOMER_ADVANCE_ID');
      ...
      return ok(res,{advance:row,finance:await loadSalesOrderFinance(row.sales_order_id)});
    }
