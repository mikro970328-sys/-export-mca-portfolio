...
function cleanLines(lines){
  ...
  return lines.map((line,index)=>{
    ...
    return {product_id:productId,ordered_quantity:text(line.ordered_quantity,80),ordered_pallets:text(line.ordered_pallets,80),units_per_pallet:text(line.units_per_pallet,80),unit_cost:text(line.unit_cost,80),notes:text(line.notes,1000)};
  });
}
...
export default async function handler(req,res){
  ...
  }catch(error){const raw=String(error.message||'No se pudo procesar Compras');console.error('[purchases]',error);return fail(res,400,translatedError(raw));}
}
