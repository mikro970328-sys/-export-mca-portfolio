...
async function listOrders() {
  const [orders, progress, items, allocations] = await Promise.all([
    ...
    supabase('purchase_order_items', { query:'?select=id,purchase_order_id,product_id,ordered_quantity,ordered_pallets,unit,units_per_pallet,unit_cost,currency,notes,created_at,updated_at,product:products(id,sku,name,brand,category,unit,package_format,default_units_per_pallet)&order=created_at.asc&limit=5000' }),
  ...
  return (orders || []).map(order => ({
    ...order,
    progress:progressByPo.get(order.id) || null,
    items:itemsByPo.get(order.id) || []
  }));
}
...
function cleanLines(lines) {
  ...
  return lines.map((line, index) => {
    ...
    return {
      ...
      unit_cost:text(line.unit_cost, 80),
...
function cleanReceiptLines(lines) {
  ...
  return lines.map((line, index) => {
    ...
    return {
      ...
      received_quantity:text(line.received_quantity, 80),
      received_pallets:text(line.received_pallets, 80),
      units_per_pallet:text(line.units_per_pallet, 80),
      net_weight_kg:text(line.net_weight_kg, 80),
      gross_weight_kg:text(line.gross_weight_kg, 80),
      lot_number:text(line.lot_number, 200),
      notes:text(line.notes, 1000)
    };
  });
}
...
function translatedError(raw) {
  const translations = [
    ['PO_SUPPLIER_NOT_FOUND','Proveedor no encontrado.'],
    ['PO_SUPPLIER_INACTIVE','El proveedor está inactivo.'],
    ['PO_WAREHOUSE_NOT_FOUND','Almacén no encontrado.'],
    ['PO_WAREHOUSE_INACTIVE','El almacén está inactivo.'],
    ['PO_PRODUCT_NOT_FOUND','Uno de los productos no existe.'],
    ['PO_PRODUCT_INACTIVE','Uno de los productos está inactivo.'],
    ['PO_HAS_INACTIVE_PRODUCT','La PO contiene un producto inactivo.'],
    ['PO_HAS_NO_ITEMS','La PO debe tener al menos una línea.'],
    ['PO_QUANTITY_REQUIRED','Cada línea debe tener cantidad o pallets.'],
    ['PO_QUANTITY_INVALID','La cantidad de la PO es inválida.'],
    ['PO_UNITS_PER_PALLET_INVALID','Las unidades por pallet son inválidas.'],
    ['PO_UNIT_COST_INVALID','El costo unitario es inválido.'],
    ['PO_NOT_DRAFT','Solo una PO en borrador puede editarse o emitirse.'],
    ['PO_NOT_ISSUED','Solo una PO emitida puede confirmarse.'],
    ['PO_ITEMS_LOCKED_BY_STATUS','Las líneas ya no pueden modificarse en el estado actual.'],
    ['PO_NOT_RECEIVABLE','La PO debe estar emitida o confirmada para recibir mercancía.'],
    ['PO_WR_WAREHOUSE_MISMATCH','El almacén receptor no coincide con la PO.'],
    ['PO_RECEIPT_MULTIPLE_SUPPLIERS','Una misma recepción no puede mezclar proveedores.'],
    ['PO_OVER_RECEIPT_REQUIRES_CONFIRMATION','La recepción excede lo ordenado y requiere confirmación explícita.'],
    ['PO_RECEIPT_QUANTITY_REQUIRED','Indica una cantidad recibida mayor que cero.'],
    ['PO_RECEIPT_QUANTITY_INVALID','La cantidad recibida es inválida.'],
    ['WR_GROSS_WEIGHT_LT_NET','El peso bruto no puede ser menor que el peso neto.'],
    ['PO_HAS_ACTIVE_RECEIPTS','No se puede cancelar una PO con recepciones activas.'],
    ['PO_CANNOT_CANCEL','La PO no puede cancelarse en su estado actual.'],
    ['PO_CANNOT_CLOSE','La PO no puede cerrarse en su estado actual.'],
    ['INVALID_PO_STATUS_TRANSITION','Transición de estado de PO no permitida.']
  ];
  ...
  try {
    ...
    if (action === 'create_plan') {
      const result = await supabase('rpc/create_purchase_order_plan', { method:'POST', body:{
        ...
        p_warehouse_id:text(body.warehouse_id, 80) || null,
        p_order_date:text(body.order_date, 40) || null,
        p_expected_at:text(body.expected_at, 80) || null,
        p_currency:text(body.currency, 10).toUpperCase() || 'USD',
        p_supplier_reference:text(body.supplier_reference, 250) || null,
        p_notes:text(body.notes, 2000) || null,
        p_actor:admin.admin_id || null
      }});
      const order = rpcRow(result);
      if (!order?.id) throw new Error('No se pudo crear la Purchase Order');
    ...
    if (action === 'replace_plan') {
      const orderId = text(body.purchase_order_id, 80);
      if (!orderId) throw new Error('Falta la Purchase Order');
      const result = await supabase('rpc/replace_purchase_order_plan', { method:'POST', body:{
      ...
      const order = rpcRow(result);
      await writeAudit(admin, 'purchase_order_updated', 'purchase_order', orderId, { po_number:order?.po_number || null });
    ...
    if (['issue','confirm','cancel','close'].includes(action)) {
      const orderId = text(body.purchase_order_id, 80);
      if (!orderId) throw new Error('Falta la Purchase Order');
      const result = await supabase('rpc/transition_purchase_order', { method:'POST', body:{ p_purchase_order_id:orderId, p_action:action } });
      const order = rpcRow(result);
    ...
    if (action === 'receive') {
      const result = await supabase('rpc/receive_purchase_order_lines', { method:'POST', body:{
        p_warehouse_id:text(body.warehouse_id, 80) || null,
        p_lines:cleanReceiptLines(body.lines),
        p_received_at:text(body.received_at, 80) || null,
        p_truck_reference:text(body.truck_reference, 250) || null,
        p_driver_name:text(body.driver_name, 250) || null,
        p_reference_number:text(body.reference_number, 500) || null,
        p_notes:text(body.notes, 2000) || null,
        p_allow_over_receipt:Boolean(body.allow_over_receipt),
        p_actor:admin.admin_id || null
      }});
      ...
      return ok(res, { receipt, orders:await listOrders() });
    }

    return fail(res, 400, 'Acción de Compras no válida');
  } catch (error) {
    const raw = String(error.message || 'No se pudo procesar Compras');
