...
function validateUpcomingShipment(row, current = {}) {
  ...
  if (category === 'upcoming_shipments' && !departureDate && !arrivalDate) throw new Error('Para Próximos envíos debes indicar al menos la fecha de salida o la fecha de llegada');
}

async function validateWorker(workerId) {
  if (!workerId) return;
  const workers = await supabase('workers', { query: `?select=id,is_active&id=eq.${encodeURIComponent(workerId)}&limit=1` });
  if (!workers?.[0]) throw new Error('El trabajador seleccionado no existe');
  if (workers[0].is_active === false) throw new Error('El trabajador seleccionado está desactivado');
}
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const rows = await supabase('commercial_publications', { query:`?select=${encodeURIComponent(SELECT_WITH_WORKER)}&order=created_at.desc` });
      const workers = await supabase('workers', { query:'?select=id,full_name,phone,position,is_active&is_active=eq.true&order=full_name.asc' });
      return ok(res, { publications:rows || [], workers:workers || [] });
    }
    if (req.method === 'POST') {
      ...
      await validateWorker(row.assigned_worker_id);
      ...
      const created = await supabase('commercial_publications', { method:'POST', query:`?select=${encodeURIComponent(SELECT_WITH_WORKER)}`, body:[row] });
      await audit('publication_created', created?.[0]?.id, { title:row.title, status:row.publication_status, assigned_worker_id:row.assigned_worker_id });
    ...
    if (req.method === 'PATCH') {
      ...
      if (row.assigned_worker_id !== undefined) await validateWorker(row.assigned_worker_id);
      row.updated_by = admin.id || null;
      const updated = await supabase('commercial_publications', { method:'PATCH', query:`?id=eq.${encodeURIComponent(id)}&select=${encodeURIComponent(SELECT_WITH_WORKER)}`, body:row });
      await audit('publication_updated', id, { title:updated?.[0]?.title || current.title, status:updated?.[0]?.publication_status || current.publication_status, assigned_worker_id:updated?.[0]?.assigned_worker_id || null });
