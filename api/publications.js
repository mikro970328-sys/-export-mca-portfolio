...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') { const body = await readJson(req); const row = clean(body); if (!row.category) throw new Error('La categoría es obligatoria'); if (!row.title) throw new Error('El título es obligatorio'); row.publication_status ||= 'draft'; row.availability_status ||= 'available'; validateUpcomingShipment(row); await validateWorker(row.assigned_worker_id); row.created_by = admin.id || null; row.updated_by = admin.id || null; if (row.publication_status === 'published') row.published_at = new Date().toISOString(); const created = await supabase('commercial_publications', { method:'POST', query:`?select=${encodeURIComponent(SELECT_WITH_WORKER)}`, body:[row] }); await audit('publication_created', created?.[0]?.id, { title:row.title, status:row.publication_status, assigned_worker_id:row.assigned_worker_id }); return ok(res, { publication:created?.[0] }); }
    if (req.method === 'PATCH') { const body = await readJson(req); const id = String(body.id || '').trim(); if (!id) return fail(res, 400, 'Falta el identificador'); const currentRows = await supabase('commercial_publications', { query:`?select=*&id=eq.${encodeURIComponent(id)}&limit=1` }); const current = currentRows?.[0]; if (!current) return fail(res, 404, 'Publicación no encontrada'); const row = clean(body, current); validateUpcomingShipment(row, current); if (row.assigned_worker_id !== undefined) await validateWorker(row.assigned_worker_id); row.updated_by = admin.id || null; const updated = await supabase('commercial_publications', { method:'PATCH', query:`?id=eq.${encodeURIComponent(id)}&select=${encodeURIComponent(SELECT_WITH_WORKER)}`, body:row }); await audit('publication_updated', id, { title:updated?.[0]?.title || current.title, status:updated?.[0]?.publication_status || current.publication_status, assigned_worker_id:updated?.[0]?.assigned_worker_id || null }); return ok(res, { publication:updated?.[0] }); }
