  ...
  try {
    if (req.method === 'GET') {
      const data = await supabase('shipments', {
        query: '?select=*,clients(id,name,company,phone,email,welcome_status,active)&order=created_at.desc'
      });
    ...
    if (req.method === 'DELETE') {
      ...
      try {
        ...
        await audit('shipment_delete_blocked_shipsgo', shipment, {
          container_number: shipment.container_number,
      ...
      await audit('shipment_deleted', shipment, {
        ...
        shipsgo_deleted: shipsgoResult.deleted,
        actor: admin.username,
        deletion_scope: 'erp_and_shipsgo'
      });
    ...
    if (req.method === 'POST') {
      ...
      const duplicate = await supabase('shipments', {
        query: `?select=id&container_number=eq.${encodeURIComponent(containerNumber)}&active=eq.true&limit=1`
      });
    ...
    if (req.method === 'PATCH') {
      ...
      for (const field of ['booking_number', 'bol_number', 'carrier', 'product', 'quantity_unit']) {
