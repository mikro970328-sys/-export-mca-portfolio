-- Preserve one version per uploaded storage object; no historical data rewrite.
...
begin
  ...
  -- The shipment lock serializes first registration and network replays.
  -- A storage object is one document, even if the browser repeats its POST.
  select d.* into v_existing
  from public.documents d
  where d.shipment_id=p_shipment_id and d.storage_bucket=p_storage_bucket
    and d.storage_path=p_storage_path and d.generated=false
  order by d.created_at,d.id
  limit 1;
  if found then
    if v_existing.deleted_at is not null then
      raise exception 'CUBA_DOCUMENT_ALREADY_DELETED';
    end if;
    if public.canonical_cuba_document_type(v_existing.document_type) is distinct from v_type
      ...
      or v_existing.file_size_bytes is distinct from p_file_size_bytes
      or v_existing.notes is distinct from p_notes then
      raise exception 'CUBA_DOCUMENT_UPLOAD_CONFLICT';
    end if;
