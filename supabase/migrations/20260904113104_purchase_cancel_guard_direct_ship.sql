-- Purchase cancellation is allowed until physical fulfillment starts.
-- Uncontainerized sales links are released automatically; a linked Direct Ship container blocks cancellation.
...
declare
  ...
  v_has_direct_shipments boolean:=false;
...
begin
  ...
  select exists(
    ...
    where poi.purchase_order_id=v_po.id
  ) into v_has_direct_shipments;

  select exists(
    select 1 from public.supplier_bills sb
    where sb.purchase_order_id=v_po.id and sb.status<>'void'
  ) or exists(
    select 1 from public.supplier_payments sp
    where sp.purchase_order_id=v_po.id and sp.status<>'reversed'
  ) into v_has_finance_links;
  ...
  v_cancel_allowed:=v_po.status in ('draft','issued','confirmed') and v_has_active_receipts is not true and v_has_direct_shipments is not true;
  ...
  return jsonb_build_object(
    ...
    'actions',jsonb_build_object(
      'edit',jsonb_build_object(
        'allowed',v_edit_allowed,
        'reason',case when v_edit_allowed then null else 'PO_NOT_EDITABLE' end,
        'mode',case when v_po.status='draft' then 'full' else 'protected' end,
        'supplier_locked',v_po.status<>'draft',
        'currency_locked',v_po.status<>'draft',
        'destination_locked',v_po.status<>'draft' and (v_has_active_receipts or v_has_supply_links),
        'has_finance_links',v_has_finance_links
      ),
      ...
      'cancel',jsonb_build_object(
        ...
        'reason',case when v_cancel_allowed then null when v_has_active_receipts then 'PO_HAS_ACTIVE_RECEIPTS' when v_has_direct_shipments then 'PO_HAS_DIRECT_SHIPMENTS' else 'PO_CANNOT_CANCEL' end,
        'has_finance_links',v_has_finance_links,
        'releases_sales_links',v_has_supply_links and v_has_direct_shipments is not true
      )
    )
  );
end;
...
begin
  ...
  if v_action='cancel' then
    delete from public.sales_procurement_allocations spa
    using public.purchase_order_items poi
    where poi.id=spa.purchase_order_item_id
...
comment on function public.transition_purchase_order(uuid,text) is
  'Canonical Purchase Order transition owner; cancellation releases uncontainerized sales links and blocks linked Direct Ship containers.';
