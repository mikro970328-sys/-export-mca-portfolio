(() => {
  ...
  function styles() {
    ...
    s.textContent = `
      ...
      .editor-error,.editor-success{padding:11px 13px;border-radius:8px;margin-bottom:15px}.editor-error{background:#fff0ef;border:1px solid #efb0aa;color:var(--bad)}.editor-success{background:#edf9f0;border:1px solid #9bd3aa;color:var(--ok)}
  ...
  function html(x) {
    return `<div class="shipment-editor-head"><div class="shipment-editor-title"><div><h2>Editar contenedor</h2><div class="shipment-editor-code">${esc(x.container_number)} · ${esc(x.carrier||'Sin naviera')}</div></div><button id="shipmentEditorClose" class="alt">Cerrar</button></div></div>
      ...
      <div class="shipment-editor-footer"><div class="muted">Los cambios se registran en la auditoría del ERP.</div><div class="shipment-editor-actions"><button id="shipmentEditorCancel" class="alt">Cancelar</button><button id="shipmentEditorSave" class="orange">Guardar cambios</button></div></div>`;
  }
  ...
  function validate() {
    ...
    if ((shipments||[]).some(s=>s.id!==current.id&&s.active!==false&&norm(s.container_number)===container)) return 'Ese número de contenedor ya está registrado en otra operación activa.';
    return '';
  }
