(() => {
  ...
  style.textContent = `
    .worker-actions-compact{display:flex!important;align-items:center;justify-content:center;position:relative;flex-wrap:nowrap!important}
    .worker-actions-compact>button:not(.worker-menu-trigger){display:none!important}
    .worker-menu-trigger{width:42px;height:38px;padding:0!important;border:1px solid #cfd8e6!important;background:#fff!important;color:#06204a!important;font-size:23px!important;line-height:1!important;border-radius:10px!important;display:inline-grid!important;place-items:center;flex:0 0 auto}
    .worker-action-popover{position:fixed;z-index:5200;width:min(280px,calc(100vw - 24px));background:#fff;border:1px solid #dfe5ee;border-radius:14px;box-shadow:0 18px 48px rgba(6,32,74,.22);padding:8px;display:none}
    ...
    @media(max-width:700px){.worker-menu-trigger{width:40px;height:36px}.worker-action-popover{width:min(290px,calc(100vw - 20px))}}
  `;
  ...
  function positionMenu(trigger){
    ...
    let left = Math.max(12, Math.min(rect.right - width, window.innerWidth - width - 12));
  ...
  function compactActions(){
    document.querySelectorAll('#workers td:last-child .actions').forEach(actions => {
      ...
      trigger.onclick = event => {
        event.stopPropagation();
        const current = [...actions.children].filter(el => el.tagName === 'BUTTON' && !el.classList.contains('worker-menu-trigger'));
