(() => {
  ...
  function workersTarget() {
    return byId('workers') || byId('workersList');
  }
  ...
  function mount() {
    ...
    if (navButton) navButton.onclick = () => {
    ...
    if (!section) {
      ...
      section.innerHTML = `<div class="grid"><section class="card"><h2>Agregar trabajador</h2><label>Nombre completo *</label><input id="workerName" placeholder="Nombre del trabajador"><label>Número de teléfono / WhatsApp *</label><input id="workerPhone" placeholder="+5351234567"><div style="margin-top:14px"><button id="saveWorker" class="orange">Guardar trabajador</button></div><div id="workerMsg" class="msg"></div></section><section class="card"><div class="section-head"><div><h2>Trabajadores registrados</h2><div class="muted">Disponibles para asignar a publicaciones, productos y operaciones futuras.</div></div><button id="reloadWorkers" class="alt">Actualizar</button></div><div id="workers">Cargando...</div></section></div>`;
      main.appendChild(section);
    }

    if (typeof titles === 'object') titles.workersSection = 'Trabajadores';
    const saveButton = byId('saveWorker');
    const reloadButton = byId('reloadWorkers');
    if (saveButton) saveButton.onclick = saveWorker;
    if (reloadButton) reloadButton.onclick = loadWorkers;
    loadWorkers();
  }
  ...
  async function loadWorkers() {
    ...
    try {
      ...
      renderWorkers();
      document.dispatchEvent(new CustomEvent('exportmca:workers-loaded', { detail: { workers } }));
    } catch (error) {
  ...
  async function saveWorker() {
    try {
      ...
      if (byId('workerPosition')) byId('workerPosition').value = '';
      await loadWorkers();
    } catch (error) {
  ...
  window.getActiveWorkers = () => workers.filter(worker => worker.is_active);
