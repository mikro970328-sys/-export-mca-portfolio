...
function addLine(seed={}){
  lineSeq++;const id=lineSeq,div=document.createElement('div');div.className='line purchase-order-line';div.dataset.line=id;
  ...
  $('orderLines').appendChild(div);div.querySelector('[data-remove-line]').onclick=()=>{div.remove();if(!$('orderLines').children.length)addLine();};div.querySelector('[data-new-product]').onclick=()=>openQuickProduct(div);div.querySelector('.lProduct').onchange=()=>syncProduct(div);syncProduct(div);
}
...
$('newOrder').onclick=openNewOrder;$('addOrderLine').onclick=()=>addLine();$('saveOrder').onclick=saveOrder;$('saveReceipt').onclick=()=>submitReceipt();$('saveQuickProduct').onclick=saveQuickProduct;$('refresh').onclick=refreshPurchases;$('search').oninput=render;
