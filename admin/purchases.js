...
let orders=[],suppliers=[],warehouses=[],products=[],view='open',editing=null,detailOrder=null,receiving=null,lineSeq=0;
...
async function load(){
  ...
  orders=d.orders||[];suppliers=d.suppliers||[];warehouses=d.warehouses||[];products=d.products||[];
  render();fillMasters();
}
function fillMasters(){
  ...
  $('oWarehouse').innerHTML=w;
  $('rWarehouse').innerHTML='<option value="">Seleccionar almacén</option>'+warehouses.map(x=>`<option value="${x.id}">${esc(x.code)} · ${esc(x.name)}</option>`).join('');
}
function filtered(){
  const q=$('search').value.trim().toLowerCase();
  return orders.filter(o=>{
    const statusOk=view==='all'||(view==='draft'?o.status==='draft':view==='closed'?['closed','cancelled'].includes(o.status):['issued','confirmed'].includes(o.status));
    const hay=[o.po_number,o.supplier?.name,o.supplier_reference,...(o.items||[]).flatMap(i=>[i.product?.sku,i.product?.name,i.product?.brand])].join(' ').toLowerCase();
    return statusOk&&(!q||hay.includes(q));
  });
}
function render(){
  const open=orders.filter(o=>['issued','confirmed'].includes(o.status)).length;
  const draft=orders.filter(o=>o.status==='draft').length;
  const received=orders.filter(o=>o.progress?.receipt_status==='received').length;
  const partial=orders.filter(o=>o.progress?.receipt_status==='partial').length;
...
function bindRows(){
  ...
  document.querySelectorAll('[data-receive-order]').forEach(b=>b.onclick=()=>openReceive(orders.find(o=>o.id===b.dataset.receiveOrder)));
}
...
function addLine(seed={}){
  ...
  div.className='line';div.dataset.line=id;
  ...
  div.querySelector('[data-remove-line]').onclick=()=>{div.remove();if(!$('orderLines').children.length)addLine()};
  div.querySelector('.lProduct').onchange=()=>syncProduct(div);
  syncProduct(div);
}
function syncProduct(div){
  const p=products.find(x=>x.id===div.querySelector('.lProduct').value);
  div.querySelector('.lProductInfo').textContent=p?[p.brand,p.unit,p.package_format].filter(Boolean).join(' · '):'';
  if(p&&!div.querySelector('.lUpp').value&&p.default_units_per_pallet) div.querySelector('.lUpp').value=p.default_units_per_pallet;
}
...
function openOrder(order=null){
  editing=order||null;
  $('orderTitle').textContent=order?`Editar ${order.po_number}`:'Nueva Purchase Order';
  $('oSupplier').value=order?.supplier_id||'';$('oWarehouse').value=order?.warehouse_id||'';$('oCurrency').value=order?.currency||'USD';
  $('oDate').value=order?.order_date||localDateToday();$('oExpected').value=toLocalInput(order?.expected_at);
  $('oReference').value=order?.supplier_reference||'';$('oNotes').value=order?.notes||'';$('orderLines').innerHTML='';
  (order?.items?.length?order.items:[{}]).forEach(i=>addLine(i));
  $('orderMsg').textContent='';$('orderModal').classList.remove('hidden');
}
function closeModal(name){$(name+'Modal').classList.add('hidden');if(name==='order')editing=null;if(name==='detail')detailOrder=null;if(name==='receive')receiving=null}
async function saveOrder(){
  const btn=$('saveOrder');
  try{
    btn.disabled=true;$('orderMsg').textContent='';
    const body={action:editing?'replace_plan':'create_plan',purchase_order_id:editing?.id,supplier_id:$('oSupplier').value,warehouse_id:$('oWarehouse').value,order_date:$('oDate').value,expected_at:localToIso($('oExpected').value),currency:$('oCurrency').value,supplier_reference:$('oReference').value,notes:$('oNotes').value,lines:collectLines()};
    await api('/api/purchases',{method:'POST',body:JSON.stringify(body)});closeModal('order');await load();
  }catch(e){$('orderMsg').textContent=e.message}finally{btn.disabled=false}
}
function itemProgress(item){
  const active=(item.allocations||[]).filter(a=>a.receipt_item?.receipt?.status==='received');
  const rq=active.reduce((s,a)=>s+n(a.received_quantity),0),rp=active.reduce((s,a)=>s+n(a.received_pallets),0);
  const complete=(n(item.ordered_quantity)===0||rq>=n(item.ordered_quantity))&&(n(item.ordered_pallets)===0||rp>=n(item.ordered_pallets));
  return {rq,rp,status:rq===0&&rp===0?'Pendiente':complete?'Recibido':'Parcial'};
}
function openDetail(id){
  detailOrder=orders.find(o=>o.id===id);if(!detailOrder)return;
  $('detailTitle').textContent=detailOrder.po_number;
  $('detailSubtitle').textContent=`${detailOrder.supplier?.name||'—'} · ${commercialLabel(detailOrder.status)} · ${receiptLabel(detailOrder.progress?.receipt_status)}`;
  const o=detailOrder;
  ...
  const acts=[];
  if(o.status==='draft')acts.push(`<button class="btn" data-detail-action="edit">Editar</button><button class="btn orange" data-detail-action="issue">Emitir PO</button><button class="btn danger" data-detail-action="cancel">Cancelar PO</button>`);
  if(o.status==='issued')acts.push(`<button class="btn primary" data-detail-action="confirm">Confirmar</button><button class="btn orange" data-detail-action="receive">Recibir</button><button class="btn" data-detail-action="close">Cerrar</button><button class="btn danger" data-detail-action="cancel">Cancelar</button>`);
  if(o.status==='confirmed')acts.push(`<button class="btn orange" data-detail-action="receive">Recibir</button><button class="btn" data-detail-action="close">Cerrar</button><button class="btn danger" data-detail-action="cancel">Cancelar</button>`);
  $('detailActions').innerHTML=acts.join('');$('detailActions').querySelectorAll('[data-detail-action]').forEach(b=>b.onclick=()=>detailAction(b.dataset.detailAction));
  $('detailMsg').textContent='';$('detailModal').classList.remove('hidden');
}
async function detailAction(action){
  if(!detailOrder)return;
  if(action==='edit'){closeModal('detail');openOrder(detailOrder);return}
  if(action==='receive'){closeModal('detail');openReceive(detailOrder);return}
...
function openReceive(order){
  receiving=order;if(!order)return;
  $('receiveTitle').textContent=`Recibir ${order.po_number}`;$('rWarehouse').value=order.warehouse_id||'';$('rReceivedAt').value=localNow();
  $('rReference').value='';$('rTruck').value='';$('rDriver').value='';$('rNotes').value='';$('receiveMsg').className='msg';$('receiveMsg').textContent='';
  $('receiveLines').innerHTML=(order.items||[]).map(i=>{const p=itemProgress(i),remQ=Math.max(0,n(i.ordered_quantity)-p.rq),remP=Math.max(0,n(i.ordered_pallets)-p.rp);return `<div class="line receive-line" data-receive-item="${i.id}"><div><div class="line-title">${esc(i.product?.sku?i.product.sku+' · ':'')}${esc(i.product?.name||'Producto')}</div><div class="receive-meta">Ordenado ${fmt(i.ordered_quantity)} ${esc(i.unit)} · Recibido ${fmt(p.rq)} · Pendiente ${fmt(remQ)}${n(i.ordered_pallets)?` · ${fmt(remP)} pallets`:''}</div></div><div><label>Recibir cantidad</label><input class="rrQty" type="number" min="0" step="0.001" value="${remQ||''}"></div><div><label>Pallets</label><input class="rrPallets" type="number" min="0" step="0.001" value="${remP||''}"></div><div><label>Lote</label><input class="rrLot"></div><input class="rrUpp" type="hidden" value="${esc(i.units_per_pallet||'')}"></div>`}).join('');
...
async function submitReceipt(allowOver=false){
  const btn=$('saveReceipt');
  try{
    btn.disabled=true;$('receiveMsg').textContent='';
    const body={action:'receive',warehouse_id:$('rWarehouse').value,received_at:localToIso($('rReceivedAt').value),reference_number:$('rReference').value,truck_reference:$('rTruck').value,driver_name:$('rDriver').value,notes:$('rNotes').value,allow_over_receipt:allowOver,lines:receiptLines()};
    const d=await api('/api/purchases',{method:'POST',body:JSON.stringify(body)});
    $('receiveMsg').className='msg ok';$('receiveMsg').textContent=`${d.receipt.receipt_number} registrada correctamente.`;await load();setTimeout(()=>closeModal('receive'),500);
  }catch(e){
    if(!allowOver&&String(e.message).includes('requiere confirmación explícita')&&confirm('La cantidad excede lo ordenado. ¿Confirmas registrar el exceso?')){btn.disabled=false;return submitReceipt(true)}
    $('receiveMsg').className='msg';$('receiveMsg').textContent=e.message;
...
$('saveReceipt').onclick=()=>submitReceipt(false);
...
['order','detail','receive'].forEach(name=>$(name+'Modal').onclick=e=>{if(e.target===$(name+'Modal'))closeModal(name)});
