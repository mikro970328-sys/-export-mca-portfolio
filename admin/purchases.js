...
function activateOrderDraft(order){orderDraft=window.ExportMcaDrafts?.register({root:document.querySelector('#orderModal .purchase-order-dialog'),key:order?`purchase-order:${order.id}`:'purchase-order:new',title:order?`edición de ${order.po_number}`:'nueva Purchase Order',capture:captureOrderDraft,restore:restoreOrderDraft})||null;}
function openOrder(order=null){
  ...
  const editCapability=capability(order,'edit');editing=order||null;editingLocks={structure:editCapability.mode==='protected',destination:editCapability.destination_locked===true,supplier:editCapability.supplier_locked===true,currency:editCapability.currency_locked===true};$('orderTitle').textContent=order?`Editar ${order.po_number}`:'Nueva Purchase Order';$('oSupplier').value=order?.supplier_id||'';$('oSupplier').disabled=editingLocks.supplier;$('oDestinationMode').disabled=editingLocks.destination;$('oWarehouse').value=order?.warehouse_id||'';setPurchaseDestination(order&&isDirectPurchase(order)?'direct':'warehouse');if(order?.warehouse_id)$('oWarehouse').value=order.warehouse_id;$('oCurrency').value=order?.currency||'USD';$('oCurrency').disabled=editingLocks.currency;$('oDate').value=order?.order_date||localDateToday();$('oExpected').value=toLocalInput(order?.expected_at);$('oReference').value=order?.supplier_reference||'';$('oNotes').value=order?.notes||'';$('addOrderLine').disabled=editingLocks.structure;$('addOrderLine').title=editingLocks.structure?'No se pueden agregar líneas después de emitir la PO':'';$('orderLines').innerHTML='';(order?.items?.length?order.items:[{}]).forEach(i=>addLine(i));$('orderMsg').textContent=editingLocks.structure?'Puedes corregir cantidades, costos, fechas, referencia y notas. Los datos ya vinculados permanecen protegidos.':'';activateOrderDraft(order);openPurchaseModal('order',editingLocks.supplier?'oDate':'oSupplier');
}
