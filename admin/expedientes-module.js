(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .exp-group-label{display:flex;align-items:center;gap:7px;font-size:12px;font-weight:800;color:#475467;margin:12px 0 8px}.exp-group-label .arrow{font-size:16px;color:#98a2b3}.exp-doc-row{display:grid;grid-template-columns:minmax(150px,1fr) minmax(160px,1.4fr) auto;gap:10px;align-items:center;padding:9px 0;border-top:1px solid #e6ebf2}.exp-doc-row:first-child{border-top:0}.exp-doc-actions{display:flex;gap:7px;justify-content:flex-end;flex-wrap:wrap}.exp-icon-action{width:34px!important;height:34px!important;min-width:34px!important;padding:0!important;display:inline-grid!important;place-items:center!important;border-radius:9px!important}.exp-icon-action svg{width:16px;height:16px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.exp-doc-shared{display:inline-block;margin-left:6px;font-size:10px;font-weight:900;color:#244a83;background:#edf3ff;border-radius:999px;padding:3px 6px;vertical-align:middle}
  ...
  function operationShipments(operation) {
    ...
    return isFinalized(operation) ? shipments : shipments.filter(shipment => !isShipmentDelivered(shipment));
  }
  ...
  function visibleDocuments(operation, documents) {
    if (isFinalized(operation)) return documents;
  ...
  function hiddenDeliveredCount(operation) { return isFinalized(operation) ? 0 : allOperationShipments(operation).filter(isShipmentDelivered).length; }
  ...
  function statusLabel(operation) {
    ...
    if (shipments.length) {
      ...
      if (shipments.every(isShipmentDelivered)) return 'Finalizado';
  ...
  function renderList() {
    ...
    let operations = state.operations.filter(operation => {
      ...
      if (state.tab === 'delivered') return isFinalized(operation);
      return !isFinalized(operation) && operation.status !== 'cancelled';
    }).filter(matchesSearch);
    operations = [...operations].sort((a,b) => operationTimestamp(b) - operationTimestamp(a));
    if (!operations.length) {
      const text = state.search.trim() ? 'No hay expedientes que coincidan con la búsqueda.' : state.tab === 'delivered' ? 'No hay expedientes finalizados.' : state.tab === 'all' ? 'No hay expedientes registrados.' : 'No hay expedientes activos.';
  ...
  function folderHtml({ id,title,meta,documents,uploadScope,uploadLabel,children='',sharedContext=null,totalCount=null }) {
  ...
  function documentFoldersHtml(operation, allDocuments) {
    ...
    ordered.forEach((group,index) => {
      ...
      html += folderHtml({ id:`exp-folder-bl-${index}`, title, meta, documents:groupDocs, uploadScope:group.bol ? `bol:${group.bol}` : null, uploadLabel:group.bol ? `B/L ${group.bol}` : null, sharedContext:context, totalCount:groupDocs.length + containerDocCount, children:`${childTitle}${containerRowsHtml(group.shipments,documents,groupLabel)}` });
    });
