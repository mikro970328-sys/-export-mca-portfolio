(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:700px){.exp-cards{grid-template-columns:1fr}.exp-folder-head,.exp-doc-row,.exp-container-line{grid-template-columns:1fr}.exp-folder-actions,.exp-doc-actions,.exp-container-actions{justify-content:flex-start}.exp-folder-body{margin-left:18px}.exp-folder-meta{margin-left:27px}.exp-client-picker-results{max-height:220px}}
    `;
  ...
  function sectionHtml() {
    return `<section class="card"><div class="toolbar"><div><h2 class="section-title">Expedientes de exportación</h2><div class="muted">Un expediente por compra o envío. El B/L se detecta desde los contenedores registrados.</div></div><button id="newExpediente" class="orange" type="button">Nuevo expediente</button></div><div class="toolbar" style="margin-top:14px;align-items:center"><div class="exp-tabs"><button id="expTabActive" class="alt active" type="button">Activos</button><button id="expTabDelivered" class="alt" type="button">Entregados</button></div><button id="reloadExpedientes" class="alt" type="button">Actualizar</button></div><input id="expedientesSearch" class="search" style="margin-top:14px" placeholder="Buscar por cliente, expediente, contenedor, producto o B/L"><div id="expedientesMsg" class="msg"></div><div id="expedientesList"></div></section>`;
  }
  ...
  function operationCard(operation) {
    ...
    return `<article class="exp-card"><div class="exp-card-head"><div><div class="exp-code">${esc(operation.operation_code || 'Expediente')}</div><div class="exp-client">${esc(client?.name || 'Cliente')}</div><div class="muted">${esc(client?.company || '')}</div></div><span class="pill ${isDelivered(operation) ? 'done' : ''}">${esc(statusLabel(operation))}</span></div><div class="exp-stats"><span class="pill">${shipments.length} contenedor${shipments.length === 1 ? '' : 'es'}${isDelivered(operation) ? '' : ' activos'}</span><span class="pill">${bols.length} B/L</span><span class="pill ${docs.length ? 'done' : ''}">${docs.length} documento${docs.length === 1 ? '' : 's'}</span>${shared.length ? `<span class="exp-shared-badge">${shared.length} B/L compartido${shared.length === 1 ? '' : 's'}</span>` : ''}</div>${hiddenDelivered ? `<div class="muted" style="margin-bottom:10px">${hiddenDelivered} contenedor${hiddenDelivered === 1 ? '' : 'es'} entregado${hiddenDelivered === 1 ? '' : 's'} oculto${hiddenDelivered === 1 ? '' : 's'} de esta vista.</div>` : ''}<button class="alt" type="button" data-open-expediente="${esc(operation.id)}">Abrir expediente</button></article>`;
  }
  function renderList() {
    const target = byId('expedientesList');
    if (!target) return;
    const operations = state.operations.filter(operation => state.tab === 'delivered' ? isDelivered(operation) : !isDelivered(operation) && operation.status !== 'cancelled').filter(matchesSearch);
  ...
  function clientSearchText(client) {
    return normalizeClientSearch([client.name, client.phone, client.company, client.mipyme_name, client.email].filter(Boolean).join(' '));
  }
  ...
  function renderClientPickerResults(query = '') {
    ...
    target.querySelectorAll('[data-exp-client-choice]').forEach(button => {
      button.onclick = () => selectExpedienteClient(button.dataset.expClientChoice);
    });
  }
  ...
  function bindExpedienteClientPicker(selectedClientId = '') {
    const search = byId('newExpedienteClientSearch');
    if (!search) return;
    if (selectedClientId) selectExpedienteClient(selectedClientId);
    else renderClientPickerResults('');
