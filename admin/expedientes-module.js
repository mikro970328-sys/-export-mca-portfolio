(() => {
  ...
  function bindExpedienteActions(operation,documents) {
    ...
    document.querySelectorAll('[data-toggle-folder]').forEach(button => {
      button.onclick=() => { const body=byId(button.dataset.toggleFolder); if (!body) return; const opening=body.classList.contains('hidden'); body.classList.toggle('hidden',!opening); button.textContent=opening ? 'Cerrar' : 'Abrir'; const arrow=byId(`${button.dataset.toggleFolder}-arrow`); if (arrow) arrow.textContent=opening ? '▼' : '▶'; };
    });
