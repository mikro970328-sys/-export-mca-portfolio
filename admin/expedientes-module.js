(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:700px){.exp-row{grid-template-columns:1fr}.exp-row-actions{grid-column:1;grid-row:auto;justify-content:flex-start}.exp-folder-head,.exp-doc-row,.exp-container-line{grid-template-columns:1fr}.exp-folder-actions,.exp-doc-actions,.exp-container-actions{justify-content:flex-start}.exp-folder-body{margin-left:18px}.exp-folder-meta{margin-left:27px}.exp-client-picker-results{max-height:220px}}
    `;
  ...
  function folderHtml({ id,title,meta,documents,uploadScope,uploadLabel,children='',sharedContext=null }) {
    return `<div class="exp-folder ${sharedContext?.shared ? 'shared' : ''}"><div class="exp-folder-head" data-folder-toggle="${esc(id)}" role="button" tabindex="0" aria-expanded="false" aria-controls="${esc(id)}"><div><div class="exp-folder-title"><span id="${esc(id)}-arrow" class="exp-folder-arrow">▶</span><span>${esc(title)}</span>${sharedContext?.shared ? `<span class="exp-shared-badge">Compartido · ${sharedContext.client_count} clientes</span>` : ''}</div><div class="exp-folder-meta">${esc(meta)}</div></div><div class="exp-folder-actions"><span class="pill ${documents.length ? 'done' : ''}">${documents.length} archivo${documents.length === 1 ? '' : 's'}</span>${uploadScope ? `<button class="orange" type="button" data-upload-scope="${esc(uploadScope)}" data-upload-label="${esc(uploadLabel || title)}">Subir</button>` : ''}</div></div><div id="${esc(id)}" class="exp-folder-body hidden">${sharedContextHtml(sharedContext)}${documentBlock(documents,'Sin archivos cargados.')}${children}</div></div>`;
  }
