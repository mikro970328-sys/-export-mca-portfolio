...
function renderStats(){const activeWh=warehouses.filter(x=>x.active).length,activeProducts=products.filter(x=>x.active).length,valid=receipts.filter(x=>x.status==='received'),pallets=valid.flatMap(x=>x.items||[]).reduce((s,x)=>s+Number(x.pallets||0),0);$('stats').innerHTML=`<div class="stat warehouse-metric-locations"><span>Almacenes activos</span><b>${activeWh}</b></div><div class="stat warehouse-metric-products"><span>Productos activos</span><b>${activeProducts}</b></div><div class="stat warehouse-metric-receipts"><span>Recepciones WR</span><b>${valid.length}</b></div><div class="stat warehouse-metric-pallets"><span>Pallets recibidos</span><b>${fmtNum(pallets)}</b></div>`;}
...
function renderReceipts(){
  ...
  const rows=list.map(r=>{
    ...
    const cancel=actionAllowed(r,'cancel')?`<button class="danger" type="button" data-cancel-receipt="${esc(r.id)}" data-receipt-number="${esc(r.receipt_number)}">Anular</button>`:'';
    return `<tr><td><b>${esc(r.receipt_number)}</b><span class="warehouse-cell-secondary">${fmtDate(r.received_at)}</span></td><td>${esc(r.warehouse?.name||'Sin almacén')}</td><td>${esc(r.supplier_name||r.supplier?.name||'Recepción directa')}</td><td>${items}</td><td>${pallets?fmtNum(pallets):'—'}</td><td><span class="pill ${r.status==='cancelled'?'cancel':'ok'}">${r.status==='cancelled'?'Anulado':'Recibido'}</span></td><td><div class="warehouse-row-actions"><button class="alt" type="button" data-view-receipt="${esc(r.id)}">Ver</button>${cancel}</div></td></tr>`;
  }).join('');
  const cards=list.map(r=>{
    const cancel=actionAllowed(r,'cancel')?`<button class="danger" type="button" data-cancel-receipt="${esc(r.id)}" data-receipt-number="${esc(r.receipt_number)}">Anular</button>`:'';
    return `<article class="receipt-card"><div class="receipt-card-top"><div><h3>${esc(r.receipt_number)}</h3><div class="receipt-card-meta">${fmtDate(r.received_at)}<br>${esc(r.warehouse?.name||'Sin almacén')} · ${esc(r.supplier_name||r.supplier?.name||'Recepción directa')}</div></div><span class="pill ${r.status==='cancelled'?'cancel':'ok'}">${r.status==='cancelled'?'Anulado':'Recibido'}</span></div><div class="receipt-card-items">${(r.items||[]).map(i=>`<div class="receipt-card-item"><b>${esc(i.product?.name||'Producto')}</b><br>${entryLabel(i)}</div>`).join('')}</div><div class="receipt-card-actions"><button class="alt" type="button" data-view-receipt="${esc(r.id)}">Ver detalle</button>${cancel}</div></article>`;
  }).join('');
  $('receiptList').innerHTML=`<div class="warehouse-list-summary"><span class="warehouse-list-count">${list.length} recepción${list.length===1?'':'es'}</span><span>${receiptView==='cancelled'?'Historial anulado':'Entradas físicas vigentes'}</span></div><div class="desktop-table warehouse-table-wrap"><table><thead><tr><th>WR</th><th>Almacén</th><th>Proveedor</th><th>Mercancía</th><th>Pallets</th><th>Estado</th><th><span class="warehouse-visually-hidden">Acciones</span></th></tr></thead><tbody>${rows}</tbody></table></div><div class="mobile-receipts">${cards}</div>`;
...
function showReceipt(id){const r=receipts.find(x=>x.id===id);if(!r)return;const itemCards=(r.items||[]).map(i=>{const pallets=Number(i.pallets||0),quantity=Number(i.quantity||0);return `<div class="receipt-item"><div class="receipt-item-head"><div><div class="receipt-item-title">${esc(i.product?.name||'Producto')}</div><div class="muted">${esc(i.product?.sku||'Sin SKU')}</div></div><b>${entryLabel(i)}</b></div><div class="receipt-item-meta">${pallets>0?`<span><b>Pallets</b>${fmtNum(i.pallets)}</span>`:''}${pallets>0&&Number(i.units_per_pallet||0)>0?`<span><b>Unid./pallet</b>${fmtNum(i.units_per_pallet)}</span>`:''}${quantity>0?`<span><b>Total unidades</b>${fmtNum(i.quantity)} ${esc(displayUnit(i))}</span>`:''}${i.lot_number?`<span><b>Lote</b>${esc(i.lot_number)}</span>`:''}${i.gross_weight_kg?`<span><b>Peso bruto</b>${fmtNum(i.gross_weight_kg)} kg</span>`:''}</div></div>`;}).join('');$('detailTitle').textContent=r.receipt_number;$('detailBody').innerHTML=`<div class="detail-grid"><div class="detail-field"><b>Almacén</b>${esc(r.warehouse?.code||'Sin código')} · ${esc(r.warehouse?.name||'Sin almacén')}</div><div class="detail-field"><b>Recepción</b>${fmtDate(r.received_at)}</div><div class="detail-field"><b>Proveedor</b>${esc(r.supplier_name||r.supplier?.name||'Recepción directa')}</div><div class="detail-field"><b>Camión / referencia</b>${esc(r.truck_reference||'Sin referencia')}</div><div class="detail-field"><b>Chofer</b>${esc(r.driver_name||'Sin registrar')}</div><div class="detail-field"><b>Referencia</b>${esc(r.reference_number||'Sin registrar')}</div></div>${r.notes?`<div class="detail-field"><b>Notas</b>${esc(r.notes)}</div>`:''}<div class="warehouse-receipt-items"><b>Mercancía recibida</b>${itemCards||'<div class="warehouse-empty"><strong>Sin líneas de mercancía</strong></div>'}</div>`;openWarehouseModal('detailModal','#closeDetail');}
