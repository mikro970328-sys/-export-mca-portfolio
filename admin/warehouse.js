...
function decision({title,message,word='ANULAR',confirmLabel='Anular recepción'}){
  return new Promise(resolve=>{
    ...
    if(!modal){
      ...
      modal.setAttribute('aria-labelledby','warehouseDecisionTitle');
      modal.setAttribute('aria-describedby','warehouseDecisionMessage');
      modal.setAttribute('aria-hidden','true');
      modal.innerHTML='<div class="modalbox compact warehouse-decision-dialog"><div class="dialog-head"><div><span class="warehouse-dialog-kicker">Confirmación operativa</span><h2 id="warehouseDecisionTitle" class="warehouse-heading-reset"></h2><div id="warehouseDecisionMessage" class="muted warehouse-decision-message"></div></div><button id="warehouseDecisionClose" class="alt icon-btn" type="button" aria-label="Cerrar confirmación">×</button></div><label id="warehouseDecisionLabel" class="warehouse-decision-label" for="warehouseDecisionInput"></label><input id="warehouseDecisionInput" autocomplete="off"><div class="actions warehouse-modal-actions"><button id="warehouseDecisionCancel" class="alt" type="button">Volver</button><button id="warehouseDecisionConfirm" class="danger" type="button" disabled></button></div></div>';
    ...
    modalTriggers.set('warehouseDecision',document.activeElement);
    ...
    const finish=value=>{
      ...
      modal.onclick=null;
      modalTriggers.get('warehouseDecision')?.focus?.();
      modalTriggers.delete('warehouseDecision');
      resolve(value);
    };
...
function renderStats(){const activeWh=warehouses.filter(x=>x.active).length,activeProducts=products.filter(x=>x.active).length,valid=receipts.filter(x=>x.status==='received'),pallets=valid.flatMap(x=>x.items||[]).reduce((s,x)=>s+Number(x.pallets||0),0);$('stats').innerHTML=`<div class="stat warehouse-metric-locations"><b>${activeWh}</b><span>Almacenes activos</span></div><div class="stat warehouse-metric-products"><b>${activeProducts}</b><span>Productos activos</span></div><div class="stat warehouse-metric-receipts"><b>${valid.length}</b><span>Recepciones WR</span></div><div class="stat warehouse-metric-pallets"><b>${fmtNum(pallets)}</b><span>Pallets recibidos</span></div>`;}
function renderWarehouses(){const content=warehouses.length?`<div class="warehouse-table-wrap"><table><thead><tr><th>Código</th><th>Almacén</th><th>Ubicación</th><th>Estado</th><th><span class="warehouse-visually-hidden">Acciones</span></th></tr></thead><tbody>${warehouses.map(x=>`<tr><td><b>${esc(x.code)}</b></td><td>${esc(x.name)}</td><td>${esc([x.city,x.country].filter(Boolean).join(', ')||'Sin ubicación')}</td><td><span class="pill ${x.active?'ok':'off'}">${x.active?'Activo':'Inactivo'}</span></td><td><div class="warehouse-row-actions">${warehouseWriteAccess?`<button class="alt" type="button" data-toggle-warehouse="${esc(x.id)}" data-active="${String(!x.active)}">${x.active?'Desactivar':'Reactivar'}</button>`:''}</div></td></tr>`).join('')}</tbody></table></div>`:'<div class="warehouse-empty"><strong>Todavía no hay almacenes</strong><span>Registra la primera ubicación para comenzar a recibir mercancía.</span></div>';$('warehouseList').innerHTML=content;$('warehouseList').querySelectorAll('[data-toggle-warehouse]').forEach(button=>button.onclick=()=>toggleWarehouse(button.dataset.toggleWarehouse,button.dataset.active==='true'));}
...
function renderProducts(){const list=filteredProducts(),content=list.length?`<div class="warehouse-list-summary"><span class="warehouse-list-count">${list.length} producto${list.length===1?'':'s'}</span><span>Catálogo visible</span></div><div class="warehouse-table-wrap"><table><thead><tr><th>SKU</th><th>Producto</th><th>Presentación</th><th>Unidad</th><th>Estado</th><th><span class="warehouse-visually-hidden">Acciones</span></th></tr></thead><tbody>${list.map(x=>`<tr><td><b>${esc(x.sku||'Sin SKU')}</b></td><td>${esc(x.name)}${x.brand?`<span class="warehouse-cell-secondary">${esc(x.brand)}</span>`:''}</td><td>${esc(x.package_format||'Sin presentación')}${x.default_units_per_pallet?`<span class="warehouse-cell-secondary">${fmtNum(x.default_units_per_pallet)} por pallet</span>`:''}</td><td>${esc(x.unit||'unidades')}</td><td><span class="pill ${x.active?'ok':'off'}">${x.active?'Activo':'Inactivo'}</span></td><td><div class="warehouse-row-actions"><button class="alt" type="button" data-toggle-product="${esc(x.id)}" data-active="${String(!x.active)}">${x.active?'Desactivar':'Reactivar'}</button></div></td></tr>`).join('')}</tbody></table></div>`:`<div class="warehouse-empty"><strong>${products.length?'No hay coincidencias':'Todavía no hay productos'}</strong><span>${products.length?'Prueba con otro SKU, nombre, marca o categoría.':'Registra el primer producto para usarlo en futuras recepciones.'}</span></div>`;$('productList').innerHTML=content;$('productList').querySelectorAll('[data-toggle-product]').forEach(button=>button.onclick=()=>toggleProduct(button.dataset.toggleProduct,button.dataset.active==='true'));}
...
function renderReceipts(){
  ...
  if(!list.length){$('receiptList').innerHTML=`<div class="warehouse-empty"><strong>${receiptView==='cancelled'?'No hay recepciones anuladas':'No hay recepciones activas'}</strong><span>${q?'No se encontraron resultados para esta búsqueda.':receiptView==='cancelled'?'Las recepciones anuladas aparecerán aquí.':'Registra una nueva recepción para iniciar el historial físico.'}</span></div>`;return;}
  const cancelButton=r=>actionAllowed(r,'cancel')?`<button class="danger" type="button" data-cancel-receipt="${esc(r.id)}" data-receipt-number="${esc(r.receipt_number)}">Anular</button>`:'';
  const state=r=>`<span class="pill ${r.status==='cancelled'?'cancel':'ok'}">${r.status==='cancelled'?'Anulado':'Recibido'}</span>`;
  const rows=list.map(r=>`<tr><td><b>${esc(r.receipt_number)}</b><span class="warehouse-cell-secondary">${fmtDate(r.received_at)}</span></td><td><b>${esc(r.warehouse?.name||'Sin almacén')}</b><span class="warehouse-cell-secondary">${esc(r.warehouse?.code||'')}</span></td><td><b>${esc(r.supplier_name||r.supplier?.name||'Recepción directa')}</b>${r.reference_number?`<span class="warehouse-cell-secondary">Ref. ${esc(r.reference_number)}</span>`:''}</td><td>${(r.items||[]).map(i=>`<div class="receipt-merchandise"><b>${esc(i.product?.name||'Producto')}</b><span class="warehouse-cell-secondary">${entryLabel(i)}</span></div>`).join('')}</td><td>${state(r)}</td><td><div class="warehouse-row-actions"><button class="alt" type="button" data-view-receipt="${esc(r.id)}">Ver</button>${cancelButton(r)}</div></td></tr>`).join('');
  const cards=list.map(r=>`<article class="receipt-card"><div class="receipt-card-top"><h3>${esc(r.receipt_number)}</h3>${state(r)}</div><div class="receipt-card-meta"><b>${esc(r.warehouse?.name||'Sin almacén')}</b><span>${fmtDate(r.received_at)}</span></div><div class="receipt-card-meta"><b>${esc(r.supplier_name||r.supplier?.name||'Recepción directa')}</b>${r.reference_number?`<span>Ref. ${esc(r.reference_number)}</span>`:''}</div><div class="receipt-card-items">${(r.items||[]).map(i=>`<div class="receipt-card-item"><b>${esc(i.product?.name||'Producto')}</b><span>${entryLabel(i)}</span></div>`).join('')}</div><div class="receipt-card-actions"><button class="alt" type="button" data-view-receipt="${esc(r.id)}">Ver detalle</button>${cancelButton(r)}</div></article>`).join('');
  $('receiptList').innerHTML=`<div class="desktop-table warehouse-table-wrap"><table><thead><tr><th>WR</th><th>Almacén</th><th>Proveedor</th><th>Mercancía</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows}</tbody></table></div><div class="mobile-receipts">${cards}</div><div class="warehouse-list-summary"><span class="warehouse-list-count">${list.length} ${list.length===1?'recepción':'recepciones'}</span><span>${receiptView==='cancelled'?'Historial anulado':'Entradas físicas vigentes'}</span></div>`;
  $('receiptList').querySelectorAll('[data-view-receipt]').forEach(button=>button.onclick=()=>showReceipt(button.dataset.viewReceipt));
  $('receiptList').querySelectorAll('[data-cancel-receipt]').forEach(button=>button.onclick=()=>cancelReceipt(button.dataset.cancelReceipt,button.dataset.receiptNumber));
}
...
function productOptions(selected=''){return '<option value="">Seleccionar producto</option>'+products.filter(x=>x.active).map(x=>`<option value="${esc(x.id)}" ${x.id===selected?'selected':''}>${esc(x.sku?x.sku+' · ':'')}${esc(x.name)}</option>`).join('');}
...
function setLineMode(div,mode){div.dataset.entryMode=mode;div.querySelectorAll('.entry-mode button').forEach(b=>{const active=b.dataset.mode===mode;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));});div.querySelector('.pallet-fields').classList.toggle('hidden',mode!=='pallets');div.querySelector('.unit-fields').classList.toggle('hidden',mode!=='units');updateLineTotal(div);}
...
function addLine(seed={}){
  lineSeq++;
  const id=lineSeq,div=document.createElement('div');
  div.className='line';div.dataset.line=id;div.dataset.entryMode=seed.entry_mode||'pallets';
  div.innerHTML=`<div class="line-head"><b>Línea ${document.querySelectorAll('#receiptLines .line').length+1}</b><button class="danger" type="button" data-remove-line="${id}">Quitar</button></div>
    <div class="grid"><div><label for="lineProduct${id}">Producto *</label><div class="product-picker"><select id="lineProduct${id}" class="line-product" aria-describedby="receiptCatalogHelp lineSummary${id}">${productOptions(seed.product_id||'')}</select><button class="alt" type="button" data-new-product="${id}">+ Nuevo</button></div></div><div><label id="lineMode${id}">Recibir por</label><div class="entry-mode" role="group" aria-labelledby="lineMode${id}"><button type="button" data-mode="pallets" aria-pressed="false">Pallets</button><button type="button" data-mode="units" aria-pressed="false">Unidades</button></div></div></div>
    <div id="lineSummary${id}" class="product-summary"></div>
    <div class="pallet-fields grid"><div><label for="linePallets${id}">Cantidad de pallets *</label><input id="linePallets${id}" class="line-pallets" type="number" min="0" step="1" value="${esc(seed.pallets||'')}"></div><div><label for="lineUpp${id}">Unidades por pallet</label><input id="lineUpp${id}" class="line-upp" type="number" min="0" step="0.001" value="${esc(seed.units_per_pallet||'')}" placeholder="Opcional para controlar unidades"></div></div>
    <div class="unit-fields hidden"><label for="lineQuantity${id}">Cantidad de unidades *</label><input id="lineQuantity${id}" class="line-quantity" type="number" min="0" step="0.001" value="${esc(seed.quantity||'')}"></div>
    <div class="grid"><div><label for="lineTotal${id}">Total recibido</label><input id="lineTotal${id}" class="line-total calculated" readonly placeholder="Se calcula cuando aplica"></div><div><label for="lineLot${id}">Lote de esta recepción</label><input id="lineLot${id}" class="line-lot" value="${esc(seed.lot_number||'')}"></div></div>
    <div class="grid3"><div><label for="lineNet${id}">Peso neto recibido (kg)</label><input id="lineNet${id}" class="line-net" type="number" min="0" step="0.001" value="${esc(seed.net_weight_kg||'')}"></div><div><label for="lineGross${id}">Peso bruto recibido (kg)</label><input id="lineGross${id}" class="line-gross" type="number" min="0" step="0.001" value="${esc(seed.gross_weight_kg||'')}"></div><div><label for="lineCost${id}">Costo unitario (USD)</label><input id="lineCost${id}" class="line-cost" type="number" min="0" step="0.0001" value="${esc(seed.unit_cost||'')}"></div></div>
    <div><label for="lineNotes${id}">Notas de esta recepción</label><input id="lineNotes${id}" class="line-notes" value="${esc(seed.notes||'')}" placeholder="Información adicional de esta línea (opcional)"></div>`;
  div.querySelector('[data-remove-line]').onclick=()=>removeLine(id);
  div.querySelector('[data-new-product]').onclick=()=>openQuickProduct(id);
  div.querySelectorAll('.entry-mode button').forEach(b=>b.onclick=()=>setLineMode(div,b.dataset.mode));
  div.querySelector('.line-product').onchange=()=>applyProductToLine(div);
  ['.line-pallets','.line-upp','.line-quantity'].forEach(sel=>div.querySelector(sel).oninput=()=>updateLineTotal(div));
  $('receiptLines').appendChild(div);setLineMode(div,div.dataset.entryMode);applyProductToLine(div);
}
function removeLine(id){
  document.querySelector(`[data-line="${id}"]`)?.remove();
  if(!$('receiptLines').children.length)addLine();
  document.querySelectorAll('#receiptLines .line').forEach((line,index)=>{line.querySelector('.line-head b').textContent=`Línea ${index+1}`;});
  $('receiptLines').querySelector('.line-product')?.focus();
}
...
function showReceipt(id){const r=receipts.find(x=>x.id===id);if(!r)return;const itemCards=(r.items||[]).map(i=>{const pallets=Number(i.pallets||0),quantity=Number(i.quantity||0);return `<div class="receipt-item"><div class="receipt-item-head"><div><div class="receipt-item-title">${esc(i.product?.name||'Producto')}</div><div class="muted">${esc(i.product?.sku||'Sin SKU')}</div></div></div><div class="receipt-item-meta">${pallets>0?`<span><b>Pallets</b>${fmtNum(i.pallets)}</span>`:''}${pallets>0&&Number(i.units_per_pallet||0)>0?`<span><b>Unid./pallet</b>${fmtNum(i.units_per_pallet)}</span>`:''}${quantity>0?`<span><b>Total unidades</b>${fmtNum(i.quantity)} ${esc(displayUnit(i))}</span>`:''}${i.lot_number?`<span><b>Lote</b>${esc(i.lot_number)}</span>`:''}${i.gross_weight_kg?`<span><b>Peso bruto</b>${fmtNum(i.gross_weight_kg)} kg</span>`:''}</div></div>`;}).join('');$('detailTitle').textContent=r.receipt_number;$('detailBody').innerHTML=`<span class="pill ${r.status==='cancelled'?'cancel':'ok'}">${r.status==='cancelled'?'Anulado':'Recibido'}</span><div class="detail-grid"><div class="detail-field"><b>Almacén</b>${esc(r.warehouse?.code||'Sin código')} · ${esc(r.warehouse?.name||'Sin almacén')}</div><div class="detail-field"><b>Recepción</b>${fmtDate(r.received_at)}</div><div class="detail-field"><b>Proveedor</b>${esc(r.supplier_name||r.supplier?.name||'Recepción directa')}</div><div class="detail-field"><b>Camión / referencia</b>${esc(r.truck_reference||'Sin referencia')}</div><div class="detail-field"><b>Chofer</b>${esc(r.driver_name||'Sin registrar')}</div><div class="detail-field"><b>Referencia</b>${esc(r.reference_number||'Sin registrar')}</div></div>${r.notes?`<div class="detail-field"><b>Notas</b>${esc(r.notes)}</div>`:''}<div class="warehouse-receipt-items"><h3>Mercancía recibida</h3>${itemCards||'<div class="warehouse-empty"><strong>Sin líneas de mercancía</strong></div>'}</div>`;openWarehouseModal('detailModal','#closeDetail');}
...
$('cancelReceiptForm').onclick=closeReceipt;
$('addLine').onclick=()=>addLine();
$('closeDetail').onclick=()=>closeWarehouseModal('detailModal');
$('closeDetailFooter').onclick=()=>closeWarehouseModal('detailModal');
$('closeQuickProduct').onclick=closeQuickProduct;
$('cancelQuickProduct').onclick=closeQuickProduct;
$('receiptSearch').oninput=renderReceipts;
$('productSearch').oninput=renderProducts;
document.querySelectorAll('[data-receipt-view]').forEach(button=>button.onclick=()=>{receiptView=button.dataset.receiptView;document.querySelectorAll('[data-receipt-view]').forEach(tab=>{const active=tab===button;tab.classList.toggle('active',active);tab.setAttribute('aria-pressed',String(active));});renderReceipts();});
document.querySelectorAll('[data-tab]').forEach(button=>button.onclick=()=>{document.querySelectorAll('[data-tab]').forEach(tab=>{const active=tab===button;tab.classList.toggle('active',active);tab.setAttribute('aria-selected',String(active));tab.tabIndex=active?0:-1;});['receipts','warehouses','products'].forEach(name=>{const active=name===button.dataset.tab;$(name+'Pane').classList.toggle('hidden',!active);$(name+'Pane').setAttribute('aria-hidden',String(!active));});});
$('receiptModal').onclick=e=>{if(e.target===$('receiptModal'))closeReceipt();};
$('detailModal').onclick=e=>{if(e.target===$('detailModal'))closeWarehouseModal('detailModal');};
$('quickProductModal').onclick=e=>{if(e.target===$('quickProductModal'))closeQuickProduct();};
document.addEventListener('keydown',event=>{
  if(event.key!=='Tab')return;
  const modal=['warehouseDecision','quickProductModal','detailModal','receiptModal'].map(id=>$(id)).find(node=>node&&!node.hidden&&!node.classList.contains('hidden'));
  if(!modal)return;
  const focusable=[...modal.querySelectorAll('button:not(:disabled),input:not(:disabled),select:not(:disabled),textarea:not(:disabled),[tabindex="0"]')].filter(node=>node.getClientRects().length);
  const first=focusable[0],last=focusable.at(-1);
  if(!first)return;
  if(event.shiftKey&&(document.activeElement===first||!modal.contains(document.activeElement))){event.preventDefault();last.focus();}
  else if(!event.shiftKey&&(document.activeElement===last||!modal.contains(document.activeElement))){event.preventDefault();first.focus();}
});
document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;const decisionModal=$('warehouseDecision');if(decisionModal&&!decisionModal.classList.contains('hidden'))return $('warehouseDecisionClose')?.click();if(!$('quickProductModal').classList.contains('hidden'))return closeQuickProduct();if(!$('detailModal').classList.contains('hidden'))return closeWarehouseModal('detailModal');if(!$('receiptModal').classList.contains('hidden'))closeReceipt();});
...
function applyEmbeddedMode(){
  ...
  if(productTab){productTab.hidden=true;productTab.setAttribute('aria-hidden','true');productTab.tabIndex=-1;}
  if($('productsPane')){$('productsPane').hidden=true;$('productsPane').setAttribute('aria-hidden','true');}
  if($('quickProductModal')){$('quickProductModal').hidden=true;$('quickProductModal').setAttribute('aria-hidden','true');}
}
