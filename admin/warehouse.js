...
function renderReceipts(){
  ...
  $('receiptList').innerHTML=`<div class="desktop-table warehouse-table-wrap"><table><thead><tr><th>WR</th><th>Almacén</th><th>Proveedor</th><th>Mercancía</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows}</tbody></table></div><div class="mobile-receipts">${cards}</div><div class="warehouse-list-summary"><span class="warehouse-list-count">${list.length} recepción${list.length===1?'':'es'}</span><span>${receiptView==='cancelled'?'Historial anulado':'Entradas físicas vigentes'}</span></div>`;
