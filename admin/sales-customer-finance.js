(() => {
  ...
  const money = (value, currency = 'USD') => {
    ...
    try {
      return new Intl.NumberFormat('en-US', { style:'currency', currency:String(currency || 'USD').toUpperCase(), maximumFractionDigits:2 }).format(n);
    } catch {
  ...
  const STATUS_LABELS = Object.freeze({
    ...
    paid:'Pagada', partial:'Pago parcial', unpaid:'Pendiente', overdue:'Vencida'
  });

  function statusClass(value) {
    if (['posted','issued','paid'].includes(value)) return 'ok';
    if (['reversed','void','overdue'].includes(value)) return 'bad';
    return 'warn';
  }
  ...
  function setBusy(value) {
    ...
    document.querySelectorAll('[data-cf-busy]').forEach(button => { button.disabled = state.busy; });
  }

  function showMessage(value, ok = false) {
    const node = byId('salesFinanceMsg');
    if (!node) return;
  ...
  function openForm({ title, subtitle = '', html, onOpen, onSave, saveLabel = 'Guardar' }) {
    ensureModals();
    byId('salesFinanceFormTitle').textContent = title;
    byId('salesFinanceFormSubtitle').textContent = subtitle;
    byId('salesFinanceFormBody').innerHTML = html;
    byId('salesFinanceFormMsg').textContent = '';
    byId('salesFinanceFormSave').textContent = saveLabel;
    byId('salesFinanceFormSave').onclick = async () => {
    ...
    byId('salesFinanceFormModal').classList.remove('hidden');
    onOpen?.();
  }

  function askAction({ title, message, acceptLabel = 'Continuar', onAccept }) {
    ensureModals();
    byId('salesFinanceDecisionTitle').textContent = title;
    byId('salesFinanceDecisionCopy').textContent = message;
    byId('salesFinanceDecisionAccept').textContent = acceptLabel;
    byId('salesFinanceDecisionMsg').textContent = '';
    byId('salesFinanceDecisionAccept').onclick = async () => {
      if (state.busy) return;
      setBusy(true);
      try {
        await onAccept();
  ...
  function metric(label, value) {
    return `<div class="sales-finance-metric"><span>${esc(label)}</span><b title="${esc(value)}">${esc(value)}</b></div>`;
  }

  function render() {
    ensureModals();
    const p = state.finance?.progress;
    if (!p) {
      byId('salesFinanceBody').innerHTML = '<div class="sales-finance-empty">No se pudo cargar el estado financiero de esta venta.</div>';
      return;
    }
    const currency = p.currency || 'USD';
    const advances = state.finance?.advances || [];
    const invoices = state.finance?.invoices || [];
    ...
    byId('salesFinanceBody').innerHTML = `
      <div class="sales-finance-metrics">
        ${metric('Venta', money(p.sales_order_total,currency))}
        ${metric('Anticipos recibidos', money(p.advance_cash_received,currency))}
        ${metric('Anticipo disponible', money(p.advance_available_amount,currency))}
        ${metric('Facturado', money(p.issued_invoice_total,currency))}
        ${metric('Cash recibido neto', money(p.cash_received_net,currency))}
      ...
      <section class="sales-finance-section">
        <div class="sales-finance-section-head"><div><h3>Facturas emitidas</h3><p>Las aplicaciones de anticipos liquidan factura pero no representan una nueva entrada de caja.</p></div></div>
        <div class="sales-finance-list">${invoices.length ? invoices.map(renderInvoice).join('') : '<div class="sales-finance-empty">No hay facturas emitidas.</div>'}</div>
      </section>`;
    bindMainEvents();
  }

  function renderInvoice(row) {
    return `<div class="sales-finance-row"><div class="sales-finance-row-head"><div><div class="sales-finance-title">${esc(row.invoice_number || 'Factura')}</div><div class="sales-finance-meta">${date(row.issue_date)} · ${status(row.payment_status)}</div></div><b>${esc(money(row.total,row.currency))}</b></div><div class="sales-finance-values"><span>Cash posterior: <b>${esc(money(row.cash_payment_amount,row.currency))}</b></span><span>Anticipo aplicado: <b>${esc(money(row.advance_applied_amount,row.currency))}</b></span><span>Saldo: <b>${esc(money(row.balance_due,row.currency))}</b></span></div></div>`;
  }
  ...
  function bindMainEvents() {
    const body = byId('salesFinanceBody');
    body.querySelector('[data-cf-register]')?.addEventListener('click', openRegisterAdvance);
    body.querySelector('[data-cf-new-proforma]')?.addEventListener('click', openNewProforma);
    body.querySelectorAll('[data-cf-apply]').forEach(button => { button.onclick = () => openApply(button.dataset.cfApply); });
    body.querySelectorAll('[data-cf-refund]').forEach(button => { button.onclick = () => openRefund(button.dataset.cfRefund); });
    body.querySelectorAll('[data-cf-reverse]').forEach(button => { button.onclick = () => openReasonAction('Reversar anticipo','Este reverso corrige un registro erróneo; no representa un reembolso al cliente.', reason => postAdvance({action:'reverse',customer_advance_id:button.dataset.cfReverse,reason})); });
    body.querySelectorAll('[data-cf-reverse-app]').forEach(button => { button.onclick = () => openReasonAction('Reversar aplicación','La factura recuperará ese saldo y el anticipo volverá a estar disponible.', reason => postAdvance({action:'reverse_application',application_id:button.dataset.cfReverseApp,reason})); });
    body.querySelectorAll('[data-cf-reverse-refund]').forEach(button => { button.onclick = () => openReasonAction('Reversar reembolso','Corrige el registro del reembolso y restaura el saldo disponible del anticipo.', reason => postAdvance({action:'reverse_refund',refund_id:button.dataset.cfReverseRefund,reason})); });
    body.querySelectorAll('[data-cf-issue-proforma]').forEach(button => { button.onclick = () => askAction({title:'Emitir Proforma',message:'La Proforma quedará emitida como snapshot comercial de la venta. No crea una factura financiera.',acceptLabel:'Emitir',onAccept:() => postProforma({action:'issue',proforma_id:button.dataset.cfIssueProforma})}); });
    body.querySelectorAll('[data-cf-void-proforma]').forEach(button => { button.onclick = () => openReasonAction('Anular Proforma','La Proforma se conservará en historial como anulada.', reason => postProforma({action:'void',proforma_id:button.dataset.cfVoidProforma,reason})); });
    body.querySelectorAll('[data-cf-print-proforma]').forEach(button => { button.onclick = () => printProforma(button.dataset.cfPrintProforma); });
  }
  ...
  function openRegisterAdvance() {
    ...
    openForm({
      ...
      subtitle:'Dinero recibido del cliente antes o independientemente de la factura.',
      html:`<div class="sales-finance-form"><div><label>Monto (${esc(currency)}) *</label><input id="cfAdvanceAmount" type="number" min="0" step="0.01"></div><div><label>Fecha recibida</label><input id="cfAdvanceDate" type="date" value="${today()}"></div><div><label>Método</label><input id="cfAdvanceMethod" placeholder="Wire, ACH, efectivo..."></div><div><label>Referencia</label><input id="cfAdvanceReference"></div><div class="full"><label>Nota</label><textarea id="cfAdvanceNotes"></textarea></div></div>`,
      onSave:() => postAdvance({action:'register',sales_order_id:state.salesOrderId,amount:byId('cfAdvanceAmount').value,received_date:byId('cfAdvanceDate').value,method:byId('cfAdvanceMethod').value,reference:byId('cfAdvanceReference').value,notes:byId('cfAdvanceNotes').value})
    });
  }
  ...
  function openApply(advanceId) {
    ...
    openForm({
      title:`Aplicar ${advance.advance_number}`,
      subtitle:`Disponible: ${money(advance.available_amount,advance.currency)}. La aplicación no mueve caja.`,
      html:`<div class="sales-finance-form"><div class="full"><label>Factura *</label><select id="cfApplyInvoice">${invoices.map(row => `<option value="${esc(row.invoice_id)}">${esc(row.invoice_number)} · saldo ${esc(money(row.balance_due,row.currency))}</option>`).join('')}</select></div><div><label>Monto *</label><input id="cfApplyAmount" type="number" min="0" step="0.01"></div><div><label>Disponible anticipo</label><input value="${esc(money(advance.available_amount,advance.currency))}" disabled></div><div class="full"><label>Nota</label><textarea id="cfApplyNotes"></textarea></div></div>`,
      onSave:() => postAdvance({action:'apply',customer_advance_id:advanceId,invoice_id:byId('cfApplyInvoice').value,amount:byId('cfApplyAmount').value,notes:byId('cfApplyNotes').value})
    });
  }
  ...
  function openRefund(advanceId) {
    ...
    openForm({
      ...
      html:`<div class="sales-finance-form"><div><label>Monto *</label><input id="cfRefundAmount" type="number" min="0" step="0.01"></div><div><label>Fecha</label><input id="cfRefundDate" type="date" value="${today()}"></div><div><label>Método</label><input id="cfRefundMethod"></div><div><label>Referencia</label><input id="cfRefundReference"></div><div class="full"><label>Nota</label><textarea id="cfRefundNotes"></textarea></div></div>`,
      onSave:() => postAdvance({action:'refund',customer_advance_id:advanceId,amount:byId('cfRefundAmount').value,refund_date:byId('cfRefundDate').value,method:byId('cfRefundMethod').value,reference:byId('cfRefundReference').value,notes:byId('cfRefundNotes').value})
    });
  }
  ...
  function openReasonAction(title, subtitle, onSubmit) {
    openForm({
      ...
      subtitle,
      saveLabel:'Confirmar',
      html:'<div class="sales-finance-form"><div class="full"><label>Motivo *</label><textarea id="cfReason" required></textarea></div></div>',
      onSave:() => {
        const reason = String(byId('cfReason').value || '').trim();
        if (!reason) throw new Error('Indica el motivo.');
  ...
  function openNewProforma() {
    ...
    openForm({
      ...
      subtitle:'Se crea desde la mercancía y precios actuales de la Sales Order como snapshot comercial.',
      html:`<div class="sales-finance-form"><div><label>Fecha</label><input id="cfProformaDate" type="date" value="${today()}"></div><div><label>Válida hasta</label><input id="cfProformaValid" type="date" value="${valid}"></div><div class="full"><label>Nota / condiciones</label><textarea id="cfProformaNotes"></textarea></div></div>`,
      onSave:() => postProforma({action:'create',sales_order_id:state.salesOrderId,issue_date:byId('cfProformaDate').value,valid_until:byId('cfProformaValid').value,notes:byId('cfProformaNotes').value})
    });
  }

  async function postAdvance(payload) {
    await request('/api/customer-advances', { method:'POST', body:JSON.stringify(payload) });
  }

  async function postProforma(payload) {
    await request('/api/proformas', { method:'POST', body:JSON.stringify(payload) });
  }

  function printProforma(id) {
    const row = state.proformas.find(item => item.id === id);
    if (!row) return;
    const clientName = row.client?.company || row.client?.mipyme_name || row.client?.name || '—';
    const importerName = row.importer?.legal_name || row.importer?.name || '—';
    const w = window.open('', '_blank', 'noopener,noreferrer');
    if (!w) {
      showMessage('El navegador bloqueó la ventana de impresión.');
      return;
    }
  ...
  function activeWorkspaceTab() {
    return byId('detailBody')?.querySelector('[data-ws-tab].active')?.dataset.wsTab || null;
  }

  function augmentBillingTab() {
    if (activeWorkspaceTab() !== 'billing') return;
    const content = byId('detailBody')?.querySelector('.sales-workspace-content');
    if (!content) return;
    byId('salesFinanceInline')?.remove();
    const p = state.finance?.progress;
    if (!p) return;
  ...
  async function refreshAll({ refreshNative = false } = {}) {
    if (refreshNative) await nativeWorkspace?.reload?.({ keepTab:true });
    await fetchData();
    render();
    augmentBillingTab();
    updateHeaderButton();
  ...
  async function open(salesOrderId) {
    state.salesOrderId = String(salesOrderId || '') || null;
    if (!state.salesOrderId) return;
    ensureModals();
    byId('salesFinanceBody').innerHTML = '<div class="sales-finance-empty">Cargando…</div>';
    byId('salesFinanceModal').classList.remove('hidden');
    try {
      await fetchData();
      render();
    } catch (error) {
  ...
  if (nativeWorkspace) {
    window.SalesWorkspace = Object.freeze({
      ...nativeWorkspace,
      open:async salesOrderId => {
        state.salesOrderId = String(salesOrderId || '') || null;
        const result = await nativeWorkspace.open(salesOrderId);
        updateHeaderButton();
        fetchData().then(augmentBillingTab).catch(() => {});
        return result;
      },
      reload:async options => {
        const result = await nativeWorkspace.reload(options);
        updateHeaderButton();
        fetchData().then(augmentBillingTab).catch(() => {});
        return result;
      },
