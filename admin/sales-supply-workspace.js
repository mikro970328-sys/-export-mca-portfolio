(() => {
  ...
  function renderProcurement(item,plan,allocation){
    ...
    const quantityDetail=plan.supply_method==='purchase_direct'
      ?`Mercancía asignada automáticamente: ${fmt(allocation.allocated_sales_quantity)} ${esc(item.unit)} · ${esc(orderStatus(po.status))}`
  ...
  function quickDirect(itemId){
    ...
    openForm({title:'Asignar Direct Ship',subtitle:`${productTitle(item)} · Pendiente ${fmt(progress.unplanned_quantity||0)} ${esc(item.unit)}`,saveLabel:'Asignar mercancía',html:`<div class="sales-supply-form"><div class="full"><label>Compra que enviará el proveedor *</label><select id="quickDirectPo"><option value="">Seleccionar compra</option>${options}</select></div><div id="quickDirectSummary" class="full sales-supply-helper">Elige una compra confirmada. El ERP asignará automáticamente el saldo disponible y sus pallets.</div><div class="full sales-supply-helper">Después podrás registrar un contenedor nuevo o usar uno existente.</div></div>`,onOpen:()=>{const select=byId('quickDirectPo'),sync=()=>{const selected=available.find(row=>row.id===select.value);byId('quickDirectSummary').textContent=selected?`${selected.purchase_order?.po_number||'Compra'} · ${fmt(selected.ordered_quantity)} ${selected.unit}. Se usará automáticamente el saldo compatible con la venta.`:'Elige una compra confirmada. El ERP asignará automáticamente el saldo disponible y sus pallets.';};select.onchange=sync;sync();},onSave:()=>request('/api/sales-supply',{method:'POST',body:JSON.stringify({action:'quick_direct',sales_order_item_id:item.id,purchase_order_item_id:byId('quickDirectPo').value})})});
  }
  ...
  function editPurchase(item,plan,allocation){
    ...
    if(plan.supply_method==='purchase_direct'){
      if(isEdit)return;
      const confirmedOptions=options.filter(row=>row.purchase_order?.status==='confirmed');
      const confirmedHtml=confirmedOptions.map(row=>`<option value="${esc(row.id)}">${esc(row.purchase_order?.po_number||'PO')} · ${esc(row.purchase_order?.supplier?.name||row.purchase_order?.supplier?.legal_name||'Proveedor')} · ${fmt(row.ordered_quantity)} ${esc(row.unit)}</option>`).join('');
      openForm({title:'Elegir compra Direct Ship',subtitle:productTitle(item),saveLabel:'Vincular compra',html:`<div class="sales-supply-form"><div class="full"><label>Compra que enviará el proveedor *</label><select id="supplyPoItem"><option value="">Seleccionar compra confirmada</option>${confirmedHtml}</select></div><div id="supplyDirectPurchaseSummary" class="full sales-supply-helper">El ERP usará automáticamente la mercancía pendiente de esta venta y el saldo disponible de la compra. No tienes que escribir cantidades.</div></div>`,onOpen:()=>{const select=byId('supplyPoItem'),sync=()=>{const selected=confirmedOptions.find(row=>row.id===select.value);byId('supplyDirectPurchaseSummary').textContent=selected?`${selected.purchase_order?.po_number||'Compra'} · ${fmt(selected.ordered_quantity)} ${selected.unit}. Se asignará automáticamente el saldo compatible.`:'El ERP usará automáticamente la mercancía pendiente de esta venta y el saldo disponible de la compra. No tienes que escribir cantidades.';};select.onchange=sync;sync();},onSave:()=>request('/api/sales-supply',{method:'POST',body:JSON.stringify({action:'quick_link_direct_purchase',supply_plan_line_id:plan.id,purchase_order_item_id:byId('supplyPoItem').value})})});
      return;
    }
