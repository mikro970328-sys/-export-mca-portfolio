(() => {
  ...
  async function open(salesOrderId=state.salesOrderId){
    ...
    byId('salesSupplyTitle').textContent='Abastecimiento';
    byId('salesSupplySubtitle').textContent='Cargando plan de abastecimiento…';
  ...
  function render(){
    ...
    byId('salesSupplyTitle').textContent=`${order.so_number} · Abastecimiento`;
    byId('salesSupplySubtitle').textContent='Planificación de origen, compras y Direct Ship. El despacho físico se registra por separado.';
    byId('salesSupplyBody').innerHTML=`<div class="sales-supply-intro"><div><strong>Una venta puede combinar varias fuentes.</strong><div>Stock existente usa Cargue/WR. Compra para almacén crea inventario solo cuando realmente se recibe. Direct Ship va del proveedor al destino sin WR ni inventario ficticio.</div></div><span class="sales-supply-status ${order.status==='confirmed'?'ok':'warn'}">${esc(orderStatus(order.status))}</span></div><div class="sales-supply-items">${items.map(renderItem).join('')}</div>`;
    bindMainActions();
  }
