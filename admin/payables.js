(() => {
  ...
  const state = { bills:[], purchaseOrders:[], payments:[], postedBills:[], paymentPOs:[], entity:'bills', view:'open', search:'', editingBillId:null, allocationPaymentId:null, reversePaymentId:null };
  ...
  function renderMetrics() {
    ...
    $('metrics').innerHTML = [
      ['Facturas',state.bills.filter(b => b.status !== 'void').length],
      ['Por pagar',money(balance)],
      ['Vencidas',overdue],
      ['Pagado',money(cashPaid)],
      ['Anticipos sin aplicar',money(unapplied)]
    ].map(([label,value]) => `<div class="metric"><b>${esc(value)}</b><span>${esc(label)}</span></div>`).join('');
  }

  function renderViewTabs() {
    const views = state.entity === 'bills'
  ...
  function renderBills() {
    ...
    return rows.map(bill => {
      ...
      if (bill.status === 'draft') actions.push(`<button class="btn" data-bill-edit="${esc(bill.id)}">Editar</button>`,`<button class="btn primary" data-bill-post="${esc(bill.id)}">Contabilizar</button>`);
      if (bill.status === 'draft' || bill.status === 'posted') actions.push(`<button class="btn danger" data-bill-void="${esc(bill.id)}">Anular</button>`);
  ...
  function renderPayments() {
    ...
    return rows.map(payment => {
      ...
      if (payment.status === 'posted') actions.push(`<button class="btn orange" data-allocate="${esc(payment.id)}">Aplicar</button>`,`<button class="btn danger" data-reverse="${esc(payment.id)}">Revertir</button>`);
  ...
  function eligiblePOs(editingBill=null) {
    return state.purchaseOrders.filter(po => po.items?.some(item => {
  ...
  function openBillEdit(id) {
    const bill=state.bills.find(row=>String(row.id)===String(id)); if (!bill || bill.status!=='draft') return;
    state.editingBillId=bill.id; $('billTitle').textContent=`Editar ${bill.bill_number}`; fillBillPOs(bill); $('bPO').value=bill.purchase_order_id; $('bPO').disabled=false;
    $('bSupplierInvoice').value=bill.supplier_invoice_number || ''; $('bDate').value=String(bill.bill_date||'').slice(0,10); $('bDue').value=String(bill.due_date||'').slice(0,10); $('bNotes').value=bill.notes || ''; renderBillLines(bill); message('billMsg',''); setModal('billModal',true);
  }
  ...
  function fillPaymentPOs() {
    $('pPO').innerHTML='<option value="">Selecciona una Purchase Order</option>'+state.paymentPOs.map(po=>`<option value="${esc(po.id)}">${esc(po.po_number)} · ${esc(supplierName(po))}</option>`).join('');
  }

  function openPaymentCreate() {
    fillPaymentPOs(); $('pPO').value=''; $('pAmount').value=''; $('pDate').value=new Date().toISOString().slice(0,10); $('pMethod').value='wire'; $('pReference').value=''; $('pNotes').value=''; message('paymentMsg',''); setModal('paymentModal',true);
  }

  async function savePayment() {
    const poId=$('pPO').value, amount=num($('pAmount').value); message('paymentMsg','');
    if (!poId) return message('paymentMsg','Selecciona una Purchase Order.'); if (amount<=0) return message('paymentMsg','El monto debe ser mayor que cero.');
    $('savePayment').disabled=true;
    try { await request('/api/supplier-payments',{method:'POST',body:JSON.stringify({action:'register',purchase_order_id:poId,amount,payment_date:$('pDate').value||null,method:$('pMethod').value||null,reference:$('pReference').value||null,notes:$('pNotes').value||null})}); setModal('paymentModal',false); state.entity='payments'; state.view='active'; await refresh(); }
    catch(error){ message('paymentMsg',error.message); } finally { $('savePayment').disabled=false; }
  }

  function openAllocation(id) {
    const payment=state.payments.find(row=>String(row.id)===String(id)); if (!payment || payment.status!=='posted') return;
    state.allocationPaymentId=payment.id; const own=new Map((payment.applications||[]).map(app=>[app.supplier_bill_id,num(app.amount)]));
    const bills=state.postedBills.filter(b=>b.purchase_order_id===payment.purchase_order_id);
    $('allocationTitle').textContent=`Aplicar ${payment.payment_number}`; $('allocationSubtitle').textContent=`${money(payment.amount,payment.currency)} · ${payment.purchase_order?.po_number || ''}`;
    ...
    message('allocationMsg',''); setModal('allocationModal',true);
  }

  async function saveAllocation() {
    const payment=state.payments.find(row=>row.id===state.allocationPaymentId); if (!payment) return;
    const applications=[...document.querySelectorAll('[data-allocation-bill]')].map(node=>({supplier_bill_id:node.dataset.allocationBill,amount:node.querySelector('[data-amount]')?.value||''})).filter(row=>num(row.amount)>0);
    const total=applications.reduce((sum,row)=>sum+num(row.amount),0); if (total>num(payment.amount)) return message('allocationMsg','La distribución supera el monto del pago.');
    $('saveAllocation').disabled=true;
    try { await request('/api/supplier-payments',{method:'POST',body:JSON.stringify({action:'replace_applications',supplier_payment_id:payment.id,applications})}); setModal('allocationModal',false); await refresh(); }
    catch(error){ message('allocationMsg',error.message); } finally { $('saveAllocation').disabled=false; }
  }

  function openReverse(id) { const payment=state.payments.find(row=>String(row.id)===String(id)); if(!payment||payment.status!=='posted')return; state.reversePaymentId=payment.id; $('rReason').value=''; message('reverseMsg',''); setModal('reverseModal',true); }
  async function saveReverse() { const reason=$('rReason').value.trim(); if(!reason)return message('reverseMsg','Indica el motivo del reverso.'); $('saveReverse').disabled=true; try{await request('/api/supplier-payments',{method:'POST',body:JSON.stringify({action:'reverse',supplier_payment_id:state.reversePaymentId,reason})});setModal('reverseModal',false);await refresh();}catch(error){message('reverseMsg',error.message);}finally{$('saveReverse').disabled=false;} }
  ...
  function openPaymentDetail(id) {
    const payment=state.payments.find(row=>String(row.id)===String(id)); if(!payment)return; const p=payment.progress||{};
    const apps=(payment.applications||[]).map(app=>{const bill=state.postedBills.find(b=>b.id===app.supplier_bill_id)||state.bills.find(b=>b.id===app.supplier_bill_id);return `<div class="detail-item"><div class="line-head"><div><b>${esc(bill?.bill_number||'Factura')}</b><div class="small">${esc(bill?.supplier_invoice_number||'')}</div></div><b>${esc(money(app.amount,payment.currency))}</b></div></div>`;}).join('');
    $('detailTitle').textContent=payment.payment_number; $('detailSubtitle').textContent=`${supplierName(payment)} · ${payment.purchase_order?.po_number||'Sin PO'} · ${date(payment.payment_date)}`;
    $('detailBody').innerHTML=`<div class="summary"><div><b>Monto</b>${esc(money(payment.amount,payment.currency))}</div><div><b>Aplicado</b>${esc(money(p.applied_amount,payment.currency))}</div><div><b>Sin aplicar</b>${esc(money(p.unapplied_amount,payment.currency))}</div><div><b>Estado</b>${paymentPill(payment)}</div></div><div class="detail-items"><b>Aplicaciones</b>${apps||'<div class="empty">Este pago todavía no está aplicado a facturas.</div>'}</div>${payment.reversal_reason?`<div class="line"><b>Motivo reverso</b><div class="small">${esc(payment.reversal_reason)}</div></div>`:''}`;
    const actions=[]; if(payment.status==='posted')actions.push(`<button class="btn orange" data-allocate="${esc(payment.id)}">Aplicar</button>`,`<button class="btn danger" data-reverse="${esc(payment.id)}">Revertir</button>`); $('detailActions').innerHTML=actions.join(''); message('detailMsg',''); setModal('detailModal',true);
  }

  async function billAction(id,action) { try{await request('/api/payables',{method:'POST',body:JSON.stringify({action,supplier_bill_id:id})});setModal('detailModal',false);await refresh();}catch(error){message('detailMsg',error.message);} }

  document.addEventListener('click',event=>{
    const el=event.target.closest('button'); if(!el)return;
    if(el.dataset.close){setModal(`${el.dataset.close}Modal`,false);return;}
    if(el.dataset.entity){state.entity=el.dataset.entity;state.view=state.entity==='bills'?'open':'active';render();return;}
    if(el.dataset.view){state.view=el.dataset.view;render();return;}
    if(el.dataset.billDetail)return openBillDetail(el.dataset.billDetail);
    if(el.dataset.paymentDetail)return openPaymentDetail(el.dataset.paymentDetail);
    if(el.dataset.billEdit){setModal('detailModal',false);return openBillEdit(el.dataset.billEdit);}
    if(el.dataset.billPost)return billAction(el.dataset.billPost,'post');
    if(el.dataset.billVoid)return billAction(el.dataset.billVoid,'void');
    if(el.dataset.allocate){setModal('detailModal',false);return openAllocation(el.dataset.allocate);}
    if(el.dataset.reverse){setModal('detailModal',false);return openReverse(el.dataset.reverse);}
  });
