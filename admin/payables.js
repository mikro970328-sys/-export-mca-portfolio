  ...
  function renderBillLines(editingBill = null) {
    ...
    const rows = (order.items || []).map(item => {
      ...
      return `<article class="payable-line" data-bill-line="${esc(item.id)}" data-pricing-mode="${pricingMode}">
        ...
        <div class="grid4"><div><label for="bill-qty-${esc(item.id)}">Cantidad</label><input id="bill-qty-${esc(item.id)}" data-qty type="number" min="0" max="${esc(available)}" step="any" value="${esc(quantity)}"></div><div><label for="bill-cost-${esc(item.id)}">Costo unitario</label><input id="bill-cost-${esc(item.id)}" data-cost type="number" min="0" step="any" value="${esc(inputNumber(cost))}"></div><div><label for="bill-total-${esc(item.id)}">Total facturado</label><input id="bill-total-${esc(item.id)}" data-total type="number" min="0" step="0.01" value="${esc(inputNumber(lineTotal, 6))}"></div><div><label for="bill-note-${esc(item.id)}">Nota</label><input id="bill-note-${esc(item.id)}" data-note value="${esc(own?.notes || '')}" placeholder="Opcional"></div></div>
      </article>`;
    }).filter(Boolean);
