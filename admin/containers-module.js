(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .container-origin{grid-column:1/-1;margin-top:18px;padding:13px 14px;border:1px solid #b8c9e4;border-radius:12px;background:#f3f7fd;display:flex;align-items:center;justify-content:space-between;gap:14px}.container-origin-label{font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.04em;color:#667085}.container-origin-title{margin-top:3px;font-size:14px;font-weight:900;color:#06204a}.container-origin-meta{margin-top:3px;font-size:11px;color:#667085}.container-origin button{white-space:nowrap}
  ...
  async function openDetails(shipment) {
    ...
    try {
      loadLink = await window.OperationalNavigation?.loadForShipment?.(shipment.id) || null;
    } catch (error) {
      console.error('[tracking load origin]', error);
    }
    const loadOriginHtml = loadLink ? `<section class="container-origin"><div><div class="container-origin-label">Origen de almacén</div><div class="container-origin-title">${esc(loadLink.load_number || 'Cargue')}</div><div class="container-origin-meta">${esc(loadLink.load_status || 'Estado no disponible')} · vinculado desde Cargues.</div></div><button id="containerOpenLoad" class="alt" type="button">Ver cargue</button></section>` : '';
