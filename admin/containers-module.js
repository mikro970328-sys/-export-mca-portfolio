(() => {
  ...
  function emptyListMessage(){if(String(byId('shipmentSearch')?.value||'').trim())return 'No hay contenedores que coincidan con la búsqueda.';if(activeFilter==='delivered')return 'Todavía no hay contenedores entregados.';if(activeFilter==='active')return 'No hay contenedores activos en este momento.';return 'No hay contenedores registrados.';}
  ...
  async function saveShipmentRecord(){
    ...
    }catch(error){console.error('CONTAINER_CREATE_FAILED',error);note(safeContainerMessage(error,'No se pudo registrar el contenedor. Revisa los datos e intenta nuevamente.'));}finally{button.disabled=false;button.textContent=original;}
  }
  ...
  async function uploadCustomsDocument(shipment,key){
    ...
    input.onchange=async()=>{const file=input.files?.[0];input.remove();if(!file)return;let prepared=null;setCustomsFeedback('Preparando carga...',true);try{const result=await request('/api/shipment-documents',{method:'POST',body:JSON.stringify({action:'prepare_upload',shipment_id:shipment.id,document_type:def.key,file_name:file.name,mime_type:file.type,file_size_bytes:file.size})});prepared=result.upload;const form=new FormData();form.append('cacheControl','3600');form.append('',file);const storageResponse=await fetch(prepared.signed_url,{method:'PUT',headers:{'x-upsert':'false'},body:form});if(!storageResponse.ok){await request('/api/shipment-documents',{method:'POST',body:JSON.stringify({action:'discard_upload',shipment_id:shipment.id,storage_path:prepared.storage_path})}).catch(()=>{});throw new Error('DOCUMENT_STORAGE_UPLOAD_FAILED');}const finalized=await request('/api/shipment-documents',{method:'POST',body:JSON.stringify({action:'finalize_upload',shipment_id:shipment.id,document_type:prepared.document_type,file_name:prepared.file_name,mime_type:prepared.mime_type,file_size_bytes:prepared.file_size_bytes,storage_path:prepared.storage_path})});await refreshAfterCustomsChange(shipment,finalized);setCustomsFeedback(`${def.label} actualizado correctamente.`,true);}catch(error){console.error('CONTAINER_DOCUMENT_UPLOAD_FAILED',{shipment_id:shipment.id,document_type:def.key,error});setCustomsFeedback(safeContainerMessage(error,'No se pudo subir el documento. Intenta nuevamente.'),false);}};
    input.click();
  }
  ...
  function notificationHistoryDetail(item){const status=String(item.delivery_status||item.status||'').toLowerCase();if(['sent','delivered','read'].includes(status))return 'Mensaje entregado correctamente.';if(['pending','queued','accepted'].includes(status))return 'Mensaje pendiente de entrega.';if(['failed','undelivered'].includes(status)||item.error_message)return 'No se pudo entregar el mensaje. Revisa el Centro de alertas.';return 'Actualización de comunicación registrada.';}
  function notificationHistoryTitle(item){const type=String(item.event_type||item.event_status||'').toLowerCase();const label=({welcome:'Bienvenida',registered:'Contenedor registrado',release:'Liberación',delivered:'Entrega',tracking:'Actualización de tracking'})[type]||'Actualización al cliente';return 'Notificación · '+label;}
  function auditHistoryDetail(item){const details=item?.details;if(typeof details==='string'&&details.trim()&&!/^[A-Z0-9_:-]{4,}$/.test(details.trim()))return details.trim();if(details&&typeof details==='object'){const value=details.reason||details.notes||details.status||details.message;if(value)return String(value);}return 'Cambio administrativo registrado.';}
  function auditHistoryTitle(item){if(item?.title)return String(item.title);return ({shipment_created:'Contenedor registrado',shipment_updated:'Contenedor actualizado',shipment_released:'Contenedor liberado',shipment_delivered:'Contenedor entregado',shipment_reactivated:'Contenedor reactivado',shipment_deleted:'Contenedor eliminado'})[String(item?.action||'')]||'Cambio administrativo';}
  async function openHistory(shipment){if(!actionAllowed(shipment,'view_history'))return;const result=await request('/api/history?shipment_id='+encodeURIComponent(shipment.id));const events=[...(result.events||[]),...(result.notifications||[]).map(item=>({title:notificationHistoryTitle(item),details:notificationHistoryDetail(item),created_at:item.created_at})),...(result.audit_events||[]).map(item=>({title:auditHistoryTitle(item),details:auditHistoryDetail(item),created_at:item.created_at}))].sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0));window.openModal?.(`Historial · ${shipment.container_number}`,events.length?`<div class="timeline">${events.map(item=>`<div class="event"><b>${esc(item.title||'Evento')}</b><div>${esc(item.details||'')}</div><div class="muted">${item.created_at?new Date(item.created_at).toLocaleString('es-US'):'—'}</div></div>`).join('')}</div>`:'<div class="empty-state">No hay historial disponible.</div>');}
  ...
  function manualResultMessage(result){const correction=result.correction_type==='rollback'?'Corrección guardada. ':'';if(result.notification_status==='failed'){if(result.notification_error)console.error('CONTAINER_TRACKING_NOTIFICATION_FAILED',{notification_error:result.notification_error});return `${correction}Estado actualizado, pero no se pudo enviar WhatsApp. Revisa el Centro de alertas.`;}if(['queued','sent','delivered','read'].includes(String(result.notification_status||'').toLowerCase()))return `${correction}Estado actualizado y WhatsApp procesado.`;if(result.notification_status==='unavailable_recipient'||result.notification_status==='pending_template')return `${correction}Estado actualizado. La notificación quedó pendiente o no disponible.`;return `${correction}Estado actualizado.`;}
