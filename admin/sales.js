...
function nationalizationLabel(s){return({nationalized:'Nacionalizada',not_nationalized:'No nacionalizada'})[s]||'Por definir'}
...
function filtered(){const q=$('search').value.trim().toLowerCase();return orders.filter(o=>{const ok=view==='all'||(view==='draft'?o.status==='draft':view==='closed'?['closed','cancelled'].includes(o.status):o.status==='confirmed');const hay=[o.so_number,clientName(o.client),o.importer?.name,o.customer_reference,nationalizationLabel(o.nationalization_status),...(o.items||[]).flatMap(i=>[i.product?.sku,i.product?.name,i.product?.brand])].join(' ').toLowerCase();return ok&&(!q||hay.includes(q))})}
function render(){
  ...
  $('metrics').innerHTML=`<article class="metric sales-metric-total"><span>Total ventas</span><b>${orders.length}</b></article><article class="metric sales-metric-draft"><span>Borradores</span><b>${draft}</b></article><article class="metric sales-metric-confirmed"><span>Confirmadas</span><b>${confirmed}</b></article><article class="metric sales-metric-progress"><span>En proceso</span><b>${partial}</b></article><article class="metric sales-metric-dispatched"><span>Despachadas</span><b>${dispatched}</b></article><div class="sales-metrics-secondary">${partial} en proceso · ${dispatched} despachadas</div>`;
  const list=filtered();
  const columns='<div class="sales-order-columns" aria-hidden="true"><span>Venta</span><span>Cliente</span><span>Valor</span><span>Estado comercial</span><span>Cumplimiento</span><span>Acciones</span></div>';
  const rowsHtml=list.length?list.map(o=>`<article class="row sales-order-row">
    <div class="sales-order-cell sales-order-identity"><span class="sales-order-cell-label">Venta</span><div class="po">${esc(o.so_number)}</div><div class="small">${date(o.order_date)}</div></div>
    <div class="sales-order-cell sales-order-client"><span class="sales-order-cell-label">Cliente</span><b>${esc(clientName(o.client))}</b><div class="small">${esc(o.importer?.name||'Sin importador')}${o.customer_reference?` · ${esc(o.customer_reference)}`:''}</div></div>
    <div class="sales-order-cell sales-order-value"><span class="sales-order-cell-label">Valor</span><b class="sales-order-total">${money(orderTotal(o),o.currency)}</b><div class="small">${(o.items||[]).length} línea(s) · ${esc(nationalizationLabel(o.nationalization_status))}</div></div>
    <div class="sales-order-cell sales-order-state"><span class="sales-order-cell-label">Estado comercial</span><span class="pill ${statusClass(o.status)}">${esc(statusLabel(o.status))}</span></div>
    <div class="sales-order-cell sales-order-fulfillment"><span class="sales-order-cell-label">Cumplimiento</span><b>${esc(fulfillmentLabel(o.progress?.fulfillment_status))}</b><div class="small">${o.requested_at?`Objetivo: ${dateTime(o.requested_at)} · `:''}${o.progress?.fully_dispatched_items||0}/${o.progress?.item_count||0} completas</div></div>
  ...
  </article>`).join(''):'<div class="empty sales-empty"><strong>No hay ventas en esta vista</strong><span>Prueba otro filtro o busca una referencia diferente.</span></div>';
  $('orderList').innerHTML=(list.length?columns:'')+rowsHtml;
  $('salesListCount').textContent=`${list.length} venta${list.length===1?'':'s'}${list.length!==orders.length?` visibles · ${orders.length} registradas`:''}`;
  $('orderList').querySelectorAll('.sales-more-actions').forEach(menu=>{
    menu.addEventListener('toggle',()=>{if(menu.open)$('orderList').querySelectorAll('.sales-more-actions').forEach(other=>{if(other!==menu)other.open=false})});
...
function addLine(seed={}){
  ...
  const fieldId=`sales-line-${lineSeq}`;
  div.innerHTML=`<div class="line-head"><div class="line-title">Línea ${$('orderLines').children.length+1}</div><button class="btn danger" type="button">Quitar</button></div>
    <div class="grid4 sales-product-grid">
      <div><label for="${fieldId}-product">Producto *</label><select id="${fieldId}-product" class="lProduct">${productOptions(seed.product_id||'')}</select></div>
      <div><label for="${fieldId}-qty">Cantidad vendida *</label><input id="${fieldId}-qty" class="lQty" type="number" min="0" step="0.001" value="${esc(seed.ordered_quantity||'')}"></div>
      <div><label for="${fieldId}-pallets">Pallets</label><input id="${fieldId}-pallets" class="lPallets" type="number" min="0" step="0.001" value="${esc(seed.ordered_pallets||'')}"></div>
      <div><label for="${fieldId}-upp">Unidades por pallet</label><input id="${fieldId}-upp" class="lUpp" type="number" min="0" step="0.001" value="${esc(seed.units_per_pallet||'')}"></div>
    </div>
    <div class="grid3 sales-price-grid"><div><label for="${fieldId}-price">Precio unitario *</label><input id="${fieldId}-price" class="lPrice" type="number" min="0" step="0.0001" value="${esc(seed.unit_price??'')}"></div></div>
    <label for="${fieldId}-notes">Notas de la línea</label><input id="${fieldId}-notes" class="lNotes" value="${esc(seed.notes||'')}" placeholder="Detalle del producto (opcional)">
    <div class="small lProductInfo"></div>`;
  $('orderLines').appendChild(div);div.querySelector('button').onclick=()=>{div.remove();if(!$('orderLines').children.length)addLine();salesOrderDraft?.touch()};div.querySelector('.lProduct').onchange=()=>syncProduct(div);syncProduct(div);window.SalesOrderUX?.mountLine?.(div);salesOrderDraft?.touch()
}
...
function captureSalesOrderDraft(){return {client_id:$('oClient').value,importer_id:$('oImporter').value,nationalization_status:$('oNationalization').value,currency:$('oCurrency').value,order_date:$('oDate').value,requested_at:$('oRequested').value,customer_reference:$('oReference').value,notes:$('oNotes').value,lines:[...document.querySelectorAll('#orderLines .line')].map(line=>({product_id:line.querySelector('.lProduct')?.value||'',ordered_quantity:line.querySelector('.lQty')?.value||'',ordered_pallets:line.querySelector('.lPallets')?.value||'',units_per_pallet:line.querySelector('.lUpp')?.value||'',unit_price:line.querySelector('.lPrice')?.value||'',line_total:line.querySelector('.lTotal')?.value||'',price_mode:line.dataset.priceMode||'',notes:line.querySelector('.lNotes')?.value||''}))};}
function restoreSalesOrderDraft(data={}){$('oClient').value=data.client_id||'';syncImporters(data.importer_id||'');$('oNationalization').value=data.nationalization_status||'';$('oCurrency').value=data.currency||'USD';$('oDate').value=data.order_date||'';$('oRequested').value=data.requested_at||'';$('oReference').value=data.customer_reference||'';$('oNotes').value=data.notes||'';$('orderLines').innerHTML='';(Array.isArray(data.lines)&&data.lines.length?data.lines:[{}]).forEach(seed=>{addLine(seed);const line=$('orderLines').lastElementChild,mode=seed.price_mode==='total'?'total':seed.price_mode==='unit'?'unit':'';if(!line)return;const price=line.querySelector('.lPrice'),total=line.querySelector('.lTotal');if(price)price.value=seed.unit_price||'';if(total)total.value=seed.line_total||'';line.dataset.priceMode=mode;if(mode==='total'&&total)total.dispatchEvent(new Event('input',{bubbles:true}));else if(mode==='unit'&&price)price.dispatchEvent(new Event('input',{bubbles:true}));});window.SalesOrderUX?.refreshTotal?.();}
function activateSalesOrderDraft(order){salesOrderDraft=window.ExportMcaDrafts?.register({root:document.querySelector('#orderModal .sales-order-dialog'),key:order?`sales-order:${order.id}`:'sales-order:new',title:order?`edición de ${order.so_number}`:'nueva venta',capture:captureSalesOrderDraft,restore:restoreSalesOrderDraft})||null;}
function clearSavedSalesOrderDraft(){salesOrderDraft?.clear({silent:true});salesOrderDraft?.destroy({flush:false});salesOrderDraft=null;}
function openOrder(order=null){salesOrderDraft?.destroy({flush:true});salesOrderDraft=null;if(order&&!can(order,'edit'))throw new Error('Esta Sales Order ya no admite edición.');if(!order&&!writeAccess)throw new Error('No tienes permiso para crear ventas.');editing=order||null;$('orderTitle').textContent=order?`Editar ${order.so_number}`:'Nueva venta';$('oClient').value=order?.client_id||'';syncImporters(order?.importer_id||'');$('oNationalization').value=order?.nationalization_status||'';$('oCurrency').value=order?.currency||'USD';$('oDate').value=order?.order_date||localDateToday();$('oRequested').value=toLocalInput(order?.requested_at);$('oReference').value=order?.customer_reference||'';$('oNotes').value=order?.notes||'';$('orderLines').innerHTML='';(order?.items?.length?order.items:[{}]).forEach(addLine);$('orderMsg').textContent='';$('orderModal').classList.remove('hidden');Promise.resolve(window.SalesOrderUX?.onOrderOpen?.(order||null)).finally(()=>{if(!$('orderModal').classList.contains('hidden')&&(editing===order||(!editing&&!order)))activateSalesOrderDraft(order);})}
function closeModal(name){$(name+'Modal').classList.add('hidden');if(name==='order'){salesOrderDraft?.destroy({flush:true});salesOrderDraft=null;editing=null}if(name==='detail')detailOrder=null;if(name==='load'){loadOrder=null;loadOptions=null}}
