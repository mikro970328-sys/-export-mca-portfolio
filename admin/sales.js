...
let orders=[],clients=[],importers=[],clientImporters=[],products=[],view='open',editing=null,detailOrder=null,lineSeq=0,loadOrder=null,loadOptions=null;
...
function hasUnallocated(o){return o?.status==='confirmed'&&(o.items||[]).some(i=>n(i.progress?.unallocated_quantity)>0||n(i.progress?.unallocated_pallets)>0)}
...
async function openLoad(order){
  ...
  }catch(e){$('loadMsg').textContent=e.message;$('loadLines').innerHTML='<div class="empty">No se pudo cargar el inventario disponible.</div>'}
}
function renderLoadLines(){
  ...
  }).join('')||'<div class="empty">Esta Sales Order ya no tiene saldo pendiente por planificar.</div>';
}
