...
function render(){
  ...
  $('metrics').innerHTML=`<div class="metric"><span>Total ventas</span><b>${orders.length}</b></div><div class="metric"><span>Borradores</span><b>${draft}</b></div><div class="metric"><span>Confirmadas</span><b>${confirmed}</b></div><div class="metric"><span>En proceso</span><b>${partial}</b></div><div class="metric"><span>Despachadas</span><b>${dispatched}</b></div>`;
  const list=filtered();
  const rowsHtml=list.length?list.map(o=>`<article class="row"><div><div class="po">${esc(o.so_number)}</div><div class="small">${date(o.order_date)}</div></div><div><b>${esc(clientName(o.client))}</b><div class="small">${esc(o.importer?.name||'Sin importador')}</div></div><div><b>${money(orderTotal(o),o.currency)}</b><div class="small">${(o.items||[]).length} línea(s)</div></div><div><span class="pill ${statusClass(o.status)}">${statusLabel(o.status)}</span></div><div><span class="pill ${fulfillmentClass(o.progress?.fulfillment_status)}">${fulfillmentLabel(o.progress?.fulfillment_status)}</span><div class="small">${o.progress?.fully_dispatched_items||0}/${o.progress?.item_count||0} completas</div></div><div>${esc(o.customer_reference||'—')}<div class="small">${dateTime(o.requested_at)}</div></div><div class="actions"><button class="btn" data-view-order="${o.id}">Ver</button>${o.status==='draft'?`<button class="btn" data-edit-order="${o.id}">Editar</button>`:''}${hasUnallocated(o)?`<button class="btn orange" data-load-order="${o.id}">Crear Cargue</button>`:''}</div></article>`).join(''):'<div class="empty">No hay ventas en esta vista.</div>';
  $('orderList').innerHTML=rowsHtml+`<div class="small" style="text-align:right;margin-top:12px;font-weight:800">${list.length} venta${list.length===1?'':'s'}${list.length!==orders.length?` visibles · ${orders.length} registradas`:''}</div>`;
...
function openOrder(order=null){editing=order||null;$('orderTitle').textContent=order?`Editar ${order.so_number}`:'Nueva venta';$('oClient').value=order?.client_id||'';syncImporters(order?.importer_id||'');$('oCurrency').value=order?.currency||'USD';$('oDate').value=order?.order_date||localDateToday();$('oRequested').value=toLocalInput(order?.requested_at);$('oReference').value=order?.customer_reference||'';$('oNotes').value=order?.notes||'';$('orderLines').innerHTML='';(order?.items?.length?order.items:[{}]).forEach(addLine);$('orderMsg').textContent='';$('orderModal').classList.remove('hidden')}
